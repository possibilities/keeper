...
- **No third-party deps, and NO `bun:sqlite`/`src/db.ts` in hooks.** Keep imports to `node:*` + dep-free `src/{dead-letter,derivers,exec-backend,grant-leaf,sidecar,doc-commit,proc-starttime}.ts`; rename may dynamically import `src/{slug,session-rename-input}.ts` plus `src/transcript/claude.ts` transitively. Never import plan plugin; Pi extensions stay isolated. Attacker-influenced payloads emit one bounded JSON line/record; no NDJSON/shell injection.
...
- **The reconciler resumes its last durable paused state** (PAUSED on a fresh board), level-triggered on
  ...
  modes, caps, cooldown, reaps, and the five reapers, read `src/autopilot-worker.ts`.
