...
Every mutating verb auto-commits its own `.planctl/` scope inline at `output.emit()` (`chore(planctl): <op> <target>`); a success envelope on stdout means the commit landed, and callers never wildcard-commit planctl state. Full contract: `docs/reference/commit-at-mutation-boundary.md`.
...
`plan:*` skills live under `skills/`; the four worker agents and the auditor/planner/scout agents live in `agents/`. Tier routing rides the emitted `worker_agent` name, the orchestrator is content-blind, and `/plan:close` runs the audit inline before the irreversible `epic close`. Full contract: `docs/reference/skills-and-agents.md`.
