...
- **Worktree mode** (durable `autopilot_state` column, default OFF, set via `set_autopilot_config` — no new RPC) **is PRODUCER-ONLY**: lanes re-derived each cycle from the DAG + live git, never a fold (see README). The recover pass's level-triggered auto-clear of a `close::<epic>` failure MUST stay scoped to recover-reason rows (`worktree-recover*`) — a genuine `finalizeEpic` close-sink block (a content conflict) shares that key and must NEVER be auto-dismissed; finalize instead DEGRADES a dirty/off-branch/non-ff shared-checkout into a `retry` skip that mints no sticky row at all (a distinct, non-`worktree-recover*` reason), so it is neither auto-cleared nor a sticky jam.
