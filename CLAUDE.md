...
- **Polyglot, single authority** — the Python CLI is the authoritative implementation; `planctl-bun` (compiled TypeScript under `src/`) is an additive port covering the read surface (`state-path`, `detect`, `status`, `epics`, `show`, `cat`, `list`, `ready`, `tasks`, `resolve-task`, `refine-context`, `validate`) the worker-loop writes (`init`, `claim`, `done`, `block`), and the epic/task editing surface (the setter family `task set-tier`/`set-description`/`set-acceptance`/`set-target-repo`/`reset`, `epic set-branch`/`set-title`/`set-primary-repo`/`set-touched-repos`, the dep editors `epic add-dep`/`add-deps`/`rm-dep`, and the short-circuiting `epic invalidate`/`queue-jump`), and the creation/deletion surface (`scaffold` mints a fresh epic tree, `epic create` mints a single epic, `refine-apply` adds/rewrites on an existing one, `epic rm` deletes an epic and every artifact it owns), proven at parity against the shared conformance suite. The ten in-wave restamp members ride one shared check-then-stamp pipeline (`src/validation_restamp.ts`) — `refine-apply` joins them (it re-stamps post-write), while `scaffold` mints its marker directly and is never a member; `claim`/`block` mutate only gitignored `state/` (zero commits); every other write self-commits byte-identical to Python in subject + trailer shape. `cat` stays format-free and `validate` emits the `{valid,errors,warnings}` envelope, both with NO invocation trailer; `refine-context --invalidate` and `validate --epic` are conditionally-mutating, self-committing only on the marker transition.
...
| Bun conformance | `bun run build && PLANCTL_BIN="$PWD/dist/planctl-bun" uv run pytest tests/test_cli.py tests/test_readonly_verbs.py tests/test_init.py tests/test_worker_verbs.py tests/test_query_verbs.py tests/test_restamp_verbs.py tests/test_creation_verbs.py tests/test_refine_apply.py tests/test_epic_rm.py tests/test_multi_repo_create_validate.py tests/test_envelope_shape.py tests/test_seed_state.py tests/test_scaffold.py tests/test_task_set_tier.py tests/test_resolve_task.py tests/test_refine_context.py tests/test_run_epic_queue_jump.py tests/test_cross_project_epic_deps.py --run-slow` — the scoped parity gate against the compiled binary, broadened to the creation/deletion surface (`--run-slow` runs `test_init.py`'s `real_git` commits and the `@integration` `test_scaffold.py`; serial, add `-n auto --dist loadscope` to parallelise; `python_only` residue skips visible). The gate stays an explicit file list rather than the full-suite `tests/` shape: the close-saga verbs (`audit`/`verdict`/`followup submit`, `close-preflight`, `close-finalize`, `epic close`) and `worker resume` are unported, so `tests/test_audit_submit.py tests/test_verdict_submit.py tests/test_followup_submit.py tests/test_close_preflight.py tests/test_close_finalize.py tests/test_epic_close.py tests/test_worker_resume.py` (plus the `epic queue-jump --help` case in `tests/test_next_skill_consistency.py`) error against the bun binary — the close-saga epic owns the collapse to `tests/` |
