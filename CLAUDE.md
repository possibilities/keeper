...
- **Eight registered hooks under `plugins/keeper/plugin/hooks/`** — events-writer (NEVER blocks), branch-guard (denies subagent branch changes/worktree-add and mutating stash; stash reads/create + daemon git allowed), grant-guard (`PreToolUse(Bash|Write|Edit|MultiEdit|NotebookEdit)`, keyed on payload `agent_type` ∈ the four escalation agents — a mutation denies unless a daemon grant leaf (`src/grant-leaf.ts`) validates the whole tuple and covers the target; unblocker writes no source, protected paths (`.git/config`/hooks/creds/MCP) survive the grant; FAILS CLOSED in jurisdiction, else inert), wrong-tree-guard (`PreToolUse(Write|Edit|MultiEdit|Bash)`, keyed on lane path `KEEPER_PLAN_WORKTREE` — denies a write outside its lane's `.git`-bounded tree; `.keeper`/temp/home/own-lane/grant allowed, `.git/config`+credentials denied; FAILS OPEN), wrapped-guard (`PreToolUse(Write|Edit|MultiEdit|NotebookEdit|SendMessage|Bash)`, keyed on `KEEPER_WRAPPED_CELL` — denies ALL source edits to a marked wrapped SUBAGENT (edit tools, in-tree Write, off-list Bash) absent a grant, plus SendMessage outright (no grant lifts it); FAILS CLOSED when marked, else inert), sidecar-writer (`PostToolUse`, owns the `~/docs` sidecar + git state, NEVER the `.md` body), docs-pusher (`Stop`, pushes `~/docs` once per turn), context-hint (`SessionStart`, node-only, surfaces the repo's `CONTEXT.md`). The events-log writer in the Pi extension (`-e`) is gated on it.
...
- **No third-party deps, and NO `bun:sqlite`/`src/db.ts` in hooks.** Keep imports to `node:*` + dep-free `src/{dead-letter,derivers,exec-backend,grant-leaf,sidecar,doc-commit,proc-starttime}.ts`. Never import plan plugin; Pi extensions stay isolated. Attacker-influenced payloads emit one bounded JSON line/record; no NDJSON/shell injection.
...
- **Sandbox ALL state classes** under the per-test tmpdir for real-state tests:
  ...
  `KEEPER_CRASH_REPORTS_DIR`, the Agent Bus pair `KEEPER_BUS_DB` / `KEEPER_BUS_SOCK`, `KEEPER_NOTES_DB`, and `KEEPER_CONFIG_DIR` — never
...
- **The reconciler resumes its last durable paused state** (PAUSED on a fresh board), level-triggered on
  ...
  modes, caps, cooldown, reaps, and the five reapers, read `src/autopilot-worker.ts`. Autopilot's
  dispatch vocabulary is `work`, `close`, and wrapped provider legs only — merge, block, and
  shared-base incidents run as confined Task subagents inside the owning `/work`/`/close` session
  (ADR 0089).
- **Worktree mode** (durable `autopilot_state` column, default OFF, via `set_autopilot_config`) **is PRODUCER-ONLY**: lanes re-derived each cycle from the DAG + live git, never a fold. The `worktree_multi_repo` flag (default OFF, same RPC) CLUSTERS a >1-toplevel epic into per-repo lane groups — one plan-close on `primary_repo` gates every group's INDEPENDENT per-repo finalize. A content conflict escalates on the bare `worktree-merge-conflict` sticky — `close::<epic>` (finalizeEpic's close-sink or recover pass-2) or `work::<taskId>` (a lane's fan-in merge) — distinct from the self-clearing `worktree-lane-premerge` guard below (vets a base before any merge). The recover pass keys each transient degrade per-(epic,repo) on `close::worktree-recover:<epic>-<repoHash>`; its `worktree-recover*` level-clear is POSITIVE-EVIDENCE-only — clears solely on a same-cycle merged/ancestor/absent resolution, else retained; tri-state probes DEFER on inconclusive. Closed/tombstoned dirt is recover-owned: after grace re-probes, spool then force; unsafe lanes page once until gone. Per-repo finalize failures key on the sibling `close::worktree-finalize:<epic>-<repoHash>` rows that never collide with recover rows; finalize DEGRADES a dirty/off-branch shared-checkout (or a lock/local-op timeout) into a non-sticky `retry` skip minting no row — but a MID-MERGE shared checkout (MERGE_HEAD) is classified DISTINCTLY: the recover pass self-heals a sole-owned `keeper/epic/*` residue via a flock-guarded `git merge --abort` (foreign/ambiguous/MERGE_AUTOSTASH residue NEVER aborted), and a persistent wedge mints a recover-level-cleared per-repo `shared-checkout-wedge` distress. A SIBLING per-repo `shared-checkout-desync` row mints past a short grace when a landed base→default merge leaves the shared checkout TRAILING the tip, cleared ONLY when a per-cycle CONTENT probe sees index AND worktree at the default tip; the live `shared-checkout-dirty` sibling mints alike. Both are PAGING operator jams — a page-once sweep pages the human ONCE per row (`human_notified_at`, re-armed by the producer level-clear), yet clear ONLY via that level-trigger, NEVER `retry_dispatch`. The fan-in LANE pre-merge rides this per-LANE: an un-losslessly-mergeable base mints a SELF-CLEARING `work::<taskId>` `worktree-lane-premerge` row (positive-evidence level-clear BY LANE PATH); a persistent wedge or immediate `abort-failed` escalates to a per-LANE `worktree-lane-wedge` `daemon` distress row (orphan-GC-exempt, DISTINCT from shared-checkout). A genuine origin-ahead non-ff is NOT a retry skip: it mints a VISIBLE `worktree-finalize-non-fast-forward` sticky for an operator to reconcile origin. A sticky `worktree-merge-conflict` row is an incident on the bounded `dispatch_failures` projection (`owner_redispatch_attempts`, `human_notified_at`; only `retry_dispatch` clears it): its owning `/work`/`/close` session claims it (`keeper incident claim`, fenced on `instance_event_id`) and runs the merge-resolver Task subagent, falling back to the deconflicter IN THE SAME session on decline — never a second dispatch. An ownerless incident re-dispatches its owning `work`/`close` verb, never an escalation verb; the human pages EXACTLY once, on terminal decline/death with no live claimant. A `SHARED_BASE_BROKEN` block routes to a per-(repo, fingerprint) daemon-elected write grant — one live claimant, ungranted owners park visibly — an in-session repairer subagent uses to land a gated trunk fix or green no-op, then unblock/ping affected tasks; trunk-red with no live claimant mints a maintenance plan task instead. Repair never dispatches a top-level session; an ungranted/expired claim pages once, only `retry_dispatch` re-arms. The cross-epic merge-gate (a dependent GROUP's lane is NOT cut until every lane-cutting SAME-RESOLVED-REPO upstream is in LOCAL default) is per-(epic,repoDir), EPHEMERAL + producer-probed ONCE/cycle as a pure `reconcile` read, minting no row — an inconclusive/error probe DEFERS, an absent lane is absent-implies-merged (teardown deletes a base once ancestor-of-default); an orphaned/squash-merged upstream surfaces via `lane_merged`+`keeper await landed`. A plan session whose recorded cwd vanished under a live recycle-checked pid mints a DETECT-ONLY `stuck-sentinel: cwd-missing` needs_human sticky — never a kill. Base-freshness is producer-only: a quiescent, threshold-drifted lane's own linked base worktree merges local default into itself, never from the fold/reconcile core; a content conflict reuses the incident/claim/subagent chain above.
