...
| Bun conformance | `bun run build && PLANCTL_BIN="$PWD/dist/planctl-bun" uv run pytest tests/test_cli.py tests/test_readonly_verbs.py tests/test_init.py tests/test_worker_verbs.py tests/test_query_verbs.py --run-slow` — the scoped parity gate against the compiled binary (`--run-slow` runs `test_init.py`'s `real_git` commit tests; serial, add `-n auto --dist loadscope` to parallelise) |
