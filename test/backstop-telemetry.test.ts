...
// ---------------------------------------------------------------------------
// fn-771 — buildMissedWakeRecord carries the true change-to-rescue latency
// ---------------------------------------------------------------------------

test("fn-771: buildMissedWakeRecord emits change_to_rescue_ms when a positive latency is supplied, leaving staleness_ms untouched", () => {
  const rec = buildMissedWakeRecord({
    ...
    now: 1000,
    lastFastPathAt: 400,
    changeToRescueMs: 90_000,
  });
  expect(rec.change_to_rescue_ms).toBe(90_000);
  // The legacy idleness signal is kept alongside (now − last_fast_path_at) for
  // the before/after shakeout — the new field does NOT replace it in the record.
  expect(rec.staleness_ms).toBe(600);
  expect(rec.last_fast_path_at).toBe(400);
});

test("fn-771: buildMissedWakeRecord clamps a NEGATIVE change_to_rescue_ms (clock skew) to null — never emits a negative latency", () => {
  const rec = buildMissedWakeRecord({
    backstop: "git-heartbeat",
    worker: "git-worker",
    fastPath: "fsevents",
    ...
    now: 1000,
    lastFastPathAt: 400,
    // committed_at_ms > now (clock skew) would compute a negative latency.
    changeToRescueMs: -50,
  });
  // Present (the arg was supplied) but clamped to null — a negative would
  // poison the histogram exactly like the idle-inflation bug being fixed.
  expect("change_to_rescue_ms" in rec).toBe(true);
  expect(rec.change_to_rescue_ms).toBeNull();
});

test("fn-771: buildMissedWakeRecord carries an explicit null change_to_rescue_ms (dirty-tree-only rescue, no commit anchor)", () => {
  const rec = buildMissedWakeRecord({
    backstop: "git-heartbeat",
    worker: "git-worker",
    fastPath: "fsevents",
    ...
    now: 1000,
    lastFastPathAt: 400,
    changeToRescueMs: null,
  });
  // The git-heartbeat always carries the field (present), null when the rescue
  // discharged no commit (a dirty-tree-only snapshot has no anchor).
  expect("change_to_rescue_ms" in rec).toBe(true);
  expect(rec.change_to_rescue_ms).toBeNull();
});

test("fn-771: buildMissedWakeRecord OMITS change_to_rescue_ms entirely when the arg is absent — non-git backstops keep the legacy record shape", () => {
  const rec = buildMissedWakeRecord({
    backstop: "plan-heartbeat",
    worker: "plan-worker",
  ...
  // No fabricated latency for a non-commit backstop — the field is absent, not
  // null, so the legacy shape is byte-exact for the other four call sites.
  expect("change_to_rescue_ms" in rec).toBe(false);
  expect(rec.staleness_ms).toBe(600);
});

test("fn-771: a change_to_rescue_ms of 0 (rescued the same ms it landed) is emitted, not clamped", () => {
  const rec = buildMissedWakeRecord({
    backstop: "git-heartbeat",
    worker: "git-worker",
    fastPath: "fsevents",
  ...
  // Boundary: 0 is a legitimate (instant) latency, not a skew sentinel.
  expect(rec.change_to_rescue_ms).toBe(0);
});
