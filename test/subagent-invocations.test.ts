...
// ---------------------------------------------------------------------------
// fn-764 — cold-blob relocation safety for the two raw fold-path bridge reads.
//
// Both findBridgePreToolUse and findPendingPreToolUseForStart now COALESCE the
// payload column over a LEFT JOIN event_blobs. These tests force-relocate every
// blob (compactColdBlobs recentRetentionMargin:0 → events.data goes NULL) and
// prove (a) the bridge still resolves and (b) a from-scratch re-fold of the
// subagent_invocations rows is byte-identical to the pre-relocation fold.
// ---------------------------------------------------------------------------

describe("fn-764 cold-blob relocation: bridge reads COALESCE", () => {
  // One agent's full PreToolUse → SubagentStart → PostToolUse → SubagentStop
  // lifecycle. Spawn order = insertion order; the driver folds it.
  function lifecycleStream(): FixtureEvent[] {
    const sess = "sess-reloc";
    const preData = JSON.stringify({
      tool_use_id: "toolu_reloc",
      tool_input: {
        description: "Investigate the relocation path",
        prompt: "a relocated prompt",
        subagent_type: "Explore",
      },
    });
    const postData = JSON.stringify({
      tool_response: { agentId: "agent_reloc" },
    });
    return [
      {
        id: 1,
  ...
  test("findBridgePreToolUse resolves after every blob is relocated (data NULL)", () => {
    const [pre] = lifecycleStream();
    insertEvent(pre);
    // margin 0 → watermark = MAX(id) → every blob moves; events.data goes NULL.
    compactColdBlobs(db, { recentRetentionMargin: 0 });
    const hot = db
      .query("SELECT data FROM events WHERE id = ?")
      .get(pre.id) as { data: string | null };
    expect(hot.data).toBeNull(); // proves the relocation actually happened

    expect(findBridgePreToolUse(db, "sess-reloc", "toolu_reloc")).toEqual({
      description: "Investigate the relocation path",
      prompt_chars: "a relocated prompt".length,
      subagent_type: "Explore",
    });
  });

  test("findPendingPreToolUseForStart resolves after every blob is relocated", () => {
    const [pre] = lifecycleStream();
    insertEvent(pre);
    compactColdBlobs(db, { recentRetentionMargin: 0 });

    expect(findPendingPreToolUseForStart(db, "sess-reloc", "Explore")).toEqual({
      description: "Investigate the relocation path",
      prompt_chars: "a relocated prompt".length,
      tool_use_id: "toolu_reloc",
    });
  ...
  test("from-scratch re-fold over a compacted DB = byte-identical subagent rows", () => {
    // (1) Pre-relocation golden: insert + drive the full lifecycle inline.
    const stream = lifecycleStream();
    for (const ev of stream) driveEvent(ev);
    const golden = projectionRows()
      .map((r) => canonicalizeRow(r))
      .join("\n");
    // The bridge actually populated the row (not a null-default fold).
    expect(golden).toContain("Investigate the relocation path");
    expect(golden).toContain('"status":"ok"');
    ...
    try {
      ...
      compactColdBlobs(db2, { recentRetentionMargin: 0 });
      // Confirm the bridge's PreToolUse blob is now off the hot column.
      const preHot = db2
        .query("SELECT data FROM events WHERE id = 1")
        .get() as { data: string | null };
      expect(preHot.data).toBeNull();

      // Re-fold from scratch: re-drive the lifecycle against db2's helpers
      // WITHOUT re-inserting events (they already exist, relocated).
      const prev = db;
      db = db2;
      try {
        for (const ev of stream) {
          if (ev.hook_event === "SubagentStart") {
          ...
          } else if (ev.hook_event === "SubagentStop") {
            const openTurnSeq = findOpenTurnForStop(
              db,
              ev.session_id,
              ev.agent_id!,
            );
            if (openTurnSeq === null) continue;
            const row = db
              .prepare(
                `SELECT ts, status FROM subagent_invocations
                  WHERE job_id = ? AND agent_id = ? AND turn_seq = ?`,
            ...
            ev.hook_event === "PostToolUse" &&
            ev.tool_name === "Agent"
          ) {
            const agentId = resolveBridgeAgentId({
              subagent_agent_id: ev.subagent_agent_id,
              data: ev.data,
            });
            ...
            const turn0 = db
              ...
              .get(ev.session_id, agentId) as { status: string } | null;
            if (turn0 === null) continue;
            const preMeta = findBridgePreToolUse(
              db,
              ev.session_id,
              ev.tool_use_id!,
            );
            ...
            db.run(
              `UPDATE subagent_invocations
                  SET subagent_type = COALESCE(?, subagent_type),
                      description = COALESCE(?, description),
                      prompt_chars = ?, tool_use_id = ?, status = ?,
                      last_event_id = ?, updated_at = ?
                WHERE job_id = ? AND agent_id = ? AND turn_seq = 0`,
              [
                preMeta?.subagent_type ?? null,
                preMeta?.description ?? null,
                preMeta?.prompt_chars ?? 0,
                ev.tool_use_id,
                nextStatus,
                ev.id,
                ev.ts,
