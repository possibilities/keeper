...
test("fn-724: SCHEMA_VERSION tracks the live schema (durable ack itself added no schema)", () => {
  ...
  // pending-PreToolUse bridge), and to 67 via fn-807 task .2 (the
  // `commit_trailer_facts` projection table that de-blobs the commit-trailer
  // channel of `syncPlanctlLinks`), each of which bumped SCHEMA_VERSION AND
  // added its version to `SUPPORTED_SCHEMA_VERSIONS` in the same commit per the
