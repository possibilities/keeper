...
test("fn-724: SCHEMA_VERSION tracks the live schema (durable ack itself added no schema)", () => {
  ...
  // And to 108 via fn-1103 task .3 (appending the nullable `harness`/`resume_target`
  // columns to BOTH events — a five-place lockstep — and jobs — migration-only;
  // an additive ALTER, NO cursor rewind: a pre-v108 stream carries neither value,
  // so a from-scratch re-fold folds both NULL byte-identical, and the fold never
  // synthesizes a harness value).
