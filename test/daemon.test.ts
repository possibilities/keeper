...
test("fn-724: SCHEMA_VERSION tracks the live schema (durable ack itself added no schema)", () => {
  ...
  // from-scratch re-fold leaves it NULL byte-identical). And to 107 via fn-1107
  // task .1 (appending the nullable `jobs.dispatch_origin` provenance column —
  // `'autopilot'` iff the SessionStart discharged a real `pending_dispatches`
  // row, else NULL; an additive ALTER, NO cursor rewind: the `Dispatched` event
  // precedes its binding SessionStart in the log, so a from-scratch re-fold
  // reproduces the same discharge and the same stamp byte-identical).
