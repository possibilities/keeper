...
const FILES = [
  ...
  "test/slow/fold-cost-bench.test.ts",
...
describe("test manifest classification", () => {
  test("maps every discovered file to one required phase", () => {
    ...
    expect(audit.files["bench-folds"]).toEqual([
      "test/slow/fold-cost-bench.test.ts",
    ]);
  ...
  test("fails closed on zero discovery, omission, missing required file, and zero phase", () => {
    ...
    expect(() =>
      auditTestManifest(
        FILES,
        TEST_MANIFEST,
        (path) => path !== "test/slow/fold-cost-bench.test.ts",
      ),
    ).toThrow("required fold-cost bench file is missing");
    expect(() =>
      auditTestManifest(
