...
// ---------------------------------------------------------------------------
// fn-650 / fn-654: autopilot exec backend (zellij-only) + `zellij_session`
// ---------------------------------------------------------------------------

test("resolveConfig: legacy `exec_backend` key is silently ignored (zellij-only backend)", () => {
  const original = process.env.KEEPER_CONFIG;
  try {
    const cfg = join(tmpDir, "config.yaml");
    // A live config carrying the retired `exec_backend: ghostty` key
    // must resolve cleanly — the key is dropped from KeeperConfig and
    // YAML's forward-compat behavior silently ignores unknowns. No
    // throw, no `execBackend` field, and the other keys still resolve.
    writeFileSync(
      cfg,
      "roots:\n  - /tmp/x\nexec_backend: ghostty\nzellij_session: legacy\n",
    );
    process.env.KEEPER_CONFIG = cfg;
    const got = resolveConfig();
    expect((got as { execBackend?: unknown }).execBackend).toBeUndefined();
    expect(got.zellijSession).toBe("legacy");
    expect(got.roots).toEqual(["/tmp/x"]);
  } finally {
    if (original === undefined) delete process.env.KEEPER_CONFIG;
    else process.env.KEEPER_CONFIG = original;
  }
...
test("resolveConfig: zellij_session / roots resolve independently from a legacy ignored key", () => {
  const original = process.env.KEEPER_CONFIG;
  try {
    const cfg = join(tmpDir, "config.yaml");
    // A retired `exec_backend` value must NOT disturb the other keys.
    writeFileSync(
      cfg,
      "roots:\n  - /tmp/projects\nzellij_session: mike\nexec_backend: tmux\n",
    );
...
test("resolveConfig: catch-block defaults include zellij_session", () => {
  const original = process.env.KEEPER_CONFIG;
  try {
    const cfg = join(tmpDir, "config.yaml");
...
test("v29 DB migrates to v30: events.planctl_queue_jump + epics.queue_jump added, idempotent re-open", () => {
  ...
  reopened.run(
    "INSERT INTO epics (epic_id, epic_number, title, status, approval, last_event_id, updated_at) VALUES ('fn-2-post30', 2, 'Post-v30', 'open', 'pending', 200, 2)",
  );
  const post = reopened
...
test("v31 DB migrates to v32: epics.default_visible added as VIRTUAL generated column; post-migrate rows compute correct values per (status, approval) (fn-634, fn-648)", () => {
  ...
  // (c) Seed the 6 corners of the (status, approval) cross product
  // post-migrate (the fn-648 v38→v39 rewind would have wiped any
  // pre-migrate rows). VIRTUAL means SQLite recomputes on every read,
  // so insert-then-read pins the CASE-wrapped expression's correctness.
  const insertPost = reopened.prepare(
    "INSERT INTO epics (epic_id, epic_number, title, status, approval, last_event_id, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
  );
  insertPost.run("e1", 1, "open+approved", "open", "approved", 1, 1);
  insertPost.run("e2", 2, "open+pending", "open", "pending", 2, 1);
  insertPost.run("e3", 3, "done+approved", "done", "approved", 3, 1);
  insertPost.run("e4", 4, "done+pending", "done", "pending", 4, 1);
  insertPost.run("e5", 5, "null+approved", null, "approved", 5, 1);
  insertPost.run("e6", 6, "null+pending", null, "pending", 6, 1);
  const rows = reopened
    .prepare("SELECT epic_id, default_visible FROM epics ORDER BY epic_id ASC")
    .all() as { epic_id: string; default_visible: number }[];
  expect(rows).toEqual([
...
test("v39 DB migrates to v40: jobs.name_history added; existing titled rows backfilled to [title]; idempotent re-open (fn-652)", () => {
  ...
  // (b) The fn-648 v38→v39 rewind WOULD wipe the seeded rows, but the
  // stored version is '39' going in, so that rewind block's guard fires
  // false (storedVersion < 39 is false: 39 < 39 is false) — the rewind is
  // skipped and the seeded rows survive for the v39→v40 backfill to
  // observe. Verify the backfill stamped name_history per the seed rule:
  // `[title]` for titled rows, `'[]'` for NULL-title rows.
  const titled = db
    .prepare("SELECT name_history FROM jobs WHERE job_id = 'j-titled'")
    .get() as { name_history: string } | null;
  expect(titled?.name_history).toBe(JSON.stringify(["hello"]));
  const nullTitle = db
    .prepare("SELECT name_history FROM jobs WHERE job_id = 'j-null-title'")
    .get() as { name_history: string } | null;
  expect(nullTitle?.name_history).toBe("[]");
  const other = db
    .prepare("SELECT name_history FROM jobs WHERE job_id = 'j-other'")
    .get() as { name_history: string } | null;
  expect(other?.name_history).toBe(JSON.stringify(["world"]));
  db.close();

  // (c) Idempotent re-open: a second openDb is a no-op (addColumnIfMissing
  // guards on presence; the v39→v40 backfill skips at stored ≥ 40).
  const { db: db2 } = openDb(dbPath);
  // Mutate one row's history; the second-open's backfill must NOT re-stamp.
  db2.run("UPDATE jobs SET name_history = ? WHERE job_id = 'j-titled'", [
  ...
  const persisted = db3
    .prepare("SELECT name_history FROM jobs WHERE job_id = 'j-titled'")
    .get() as { name_history: string } | null;
  expect(persisted?.name_history).toBe(JSON.stringify(["hello", "goodbye"]));
  const ver3 = db3
    .prepare("SELECT value FROM meta WHERE key = 'schema_version'")
    .get() as { value: string };
  expect(ver3.value).toBe("41");
  db3.close();
});
...
test("v40 DB migrates to v41: usage.rate_limit_lifts_at + usage.last_usage_fold_at added; existing rows survive NULL; idempotent re-open (fn-651)", () => {
  ...
  // (b) No backfill — existing rows carry NULL on both new columns.
  const claude = db
    .prepare(
      "SELECT rate_limit_lifts_at, last_usage_fold_at FROM usage WHERE id = 'claude-default'",
    )
    .get() as {
    rate_limit_lifts_at: string | null;
    last_usage_fold_at: number | null;
  } | null;
  expect(claude?.rate_limit_lifts_at).toBeNull();
  expect(claude?.last_usage_fold_at).toBeNull();
  const codex = db
    .prepare(
      "SELECT rate_limit_lifts_at, last_usage_fold_at FROM usage WHERE id = 'codex-default'",
    )
    .get() as {
    rate_limit_lifts_at: string | null;
    last_usage_fold_at: number | null;
  } | null;
  expect(codex?.rate_limit_lifts_at).toBeNull();
  expect(codex?.last_usage_fold_at).toBeNull();
  db.close();

  // (c) Idempotent re-open: a second openDb is a no-op (addColumnIfMissing
  // guards on presence; no backfill block to re-run).
  const { db: db2 } = openDb(dbPath);
  // Stamp values on one row so we can verify the second open did not wipe them.
  db2.run(
    "UPDATE usage SET rate_limit_lifts_at = ?, last_usage_fold_at = ? WHERE id = 'claude-default'",
