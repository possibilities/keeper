...
/**
 * Deliver a single readiness "all-nine-empty" frame batch under the
 ...
 * `scheduled_tasks` as the 9th (the jobs-TUI cron detail feed).
 */
...
/**
 * Deliver a nine-collection frame where `epics` carries one row.
 */
...
/**
 * Deliver a nine-collection readiness frame carrying explicit git + jobs
...
// ---------------------------------------------------------------------------
// fn-857: `complete <epic>` met carries `followup=<id>` for closer-minted
// children (read off `created_by_closer_of`). Built off the present-then-drop
// re-query template above; the drop frame carries the child epic(s) so
// `latestReadiness` holds them at the met-commit scan.
// ---------------------------------------------------------------------------

/**
 * A child-epic row whose `created_by_closer_of` points at the awaited parent
 * (`fn-1-foo`). Defaults to a single child slotted directly after the parent.
 */
function makeChildEpicRow(
  overrides: Record<string, unknown> = {},
): Record<string, unknown> {
  return makeEpicRow({
    epic_id: "fn-2-child",
    epic_number: 2,
    created_by_closer_of: "fn-1-foo",
    sort_path: "000001.000002",
    ...overrides,
  });
}

/**
 * Drive the present-then-drop + re-query-hit path, with the drop frame
 * carrying `dropEpics` (the parent gone, children optionally present), and
 * return the harness so the caller can assert on the terminal met line.
 */
async function runCompleteEpicWithDrop(opts: {
  target: string;
  dropEpics: Record<string, unknown>[];
  json?: boolean;
}): Promise<AwaitHarness> {
  const { factory, socketRef, socketsAll } = makeMockConnect();
  const h = makeHarness(factory);
  const idPrefix = `await-${process.pid}`;

  await runAndCatch(
    singleArgs("complete", opts.target, "epic", { json: opts.json ?? false }),
    h.deps,
  );
  ...
  // Drop frame: parent gone, children (if any) present. `latestReadiness`
  // becomes this frame, so the met scan sees the children.
  deliverFiveWith(sock, idPrefix, { epics: opts.dropEpics, rev: 2 });
...
test("fn-857: complete-epic met carries followup=<id> for one closer-child", async () => {
  const h = await runCompleteEpicWithDrop({
    target: "fn-1-foo",
    dropEpics: [makeChildEpicRow({})],
  });
  expect(h.exitCode).toBe(0);
  const met = h.stdout.find((l) => l.includes("[keeper-await] met"));
  expect(met).toBeDefined();
  expect(met).toContain("followup=fn-2-child");
});

test("fn-857: complete-epic met with NO child stays byte-identical (no followup token)", async () => {
  const h = await runCompleteEpicWithDrop({
    target: "fn-1-foo",
    dropEpics: [],
  });
...
test("fn-857: multiple children → comma-joined in sort_path order", async () => {
  const h = await runCompleteEpicWithDrop({
    target: "fn-1-foo",
    // Delivered out of sort_path order; the met must re-sort.
    dropEpics: [
      makeChildEpicRow({
  ...
  expect(h.exitCode).toBe(0);
  const met = h.stdout.find((l) => l.includes("[keeper-await] met"));
  expect(met).toContain("followup=fn-7-child-a,fn-9-child-b");
});

test("fn-857: --json met emits followup as an array", async () => {
  const h = await runCompleteEpicWithDrop({
    target: "fn-1-foo",
    dropEpics: [makeChildEpicRow({})],
    json: true,
  ...
  expect(h.exitCode).toBe(0);
  const met = h.stdout.find((l) => l.includes('"event":"met"'));
  expect(met).toBeDefined();
  const parsed = JSON.parse(met ?? "{}");
  expect(parsed.followup).toEqual(["fn-2-child"]);
});

test("fn-857: --json met omits followup key when no child", async () => {
  const h = await runCompleteEpicWithDrop({
    target: "fn-1-foo",
    dropEpics: [],
    json: true,
  ...
  const met = h.stdout.find((l) => l.includes('"event":"met"'));
  const parsed = JSON.parse(met ?? "{}");
  expect("followup" in parsed).toBe(false);
});

test("fn-857: bare fn-N await still resolves the child (resolvedEpicId path)", async () => {
  // A bare `fn-N` target defers a re-query on EVERY present frame (the
  // `needsReQuery` `.some(epic_id===id)` check never matches a bare id), so
  // the socket sequence differs from the full-id path: sockets[1] is the
  // present-frame re-query, sockets[2] the drop-frame one. The resolvedEpicId
  // stamp (full `fn-1-foo`) is what lets the met scan find the child.
  const { factory, socketRef, socketsAll } = makeMockConnect();
  const h = makeHarness(factory);
  const idPrefix = `await-${process.pid}`;
  const reSubId = `await-requery-${process.pid}-epics`;

  await runAndCatch(singleArgs("complete", "fn-1", "epic"), h.deps);
  ...
  // Present frame: bare `fn-1` resolves to `fn-1-foo` (stamps resolvedEpicId),
  // but the present-frame defer opens a re-query. Resolve it as a HIT — the
  // epic is still present, so the re-eval reads `waiting`, NOT a terminal.
  deliverFiveWithEpic(sock, idPrefix, makeEpicRow({}));
  await Promise.resolve();
  await Promise.resolve();
  const presentReSock = socketsAll.sockets[1];
  if (!presentReSock) {
    throw new Error("present-frame re-query socket not opened");
  }
  presentReSock.takeOutbound();
  presentReSock.deliver([resultFrame("epics", reSubId, [makeEpicRow({})])]);
  await Promise.resolve();
  await Promise.resolve();
  expect(h.exitCode).toBeNull(); // still on board → no terminal yet

  // Drop frame: parent gone, child present in latestReadiness.
  deliverFiveWith(sock, idPrefix, { epics: [makeChildEpicRow({})], rev: 2 });
  await Promise.resolve();
  await Promise.resolve();
  const dropReSock = socketsAll.sockets[2];
  if (!dropReSock) {
    throw new Error("drop-frame re-query socket not opened");
  }
  dropReSock.takeOutbound();
  dropReSock.deliver([resultFrame("epics", reSubId, [makeEpicRow({})])]);
  await Promise.resolve();
  await Promise.resolve();
...
test("fn-857: unblocked epic met carries NO followup", async () => {
  const { factory, socketRef } = makeMockConnect();
  const h = makeHarness(factory);
  const idPrefix = `await-${process.pid}`;
  ...
  // A ready epic (workable task) WITH a closer child also present → unblocked
  // fires met on the present branch; the `complete`-only gate must keep
  // followup OFF this met even though a child exists.
  const taskReady = makeTaskRow({ approval: "approved" });
...
test("server-up: opens a readiness subscribe and fires met on first snapshot", async () => {
  ...
  // server-up rides a full readiness subscribe (nine collections — fn-770
  // added `autopilot_state` + `armed_epics`; fn-813 added `scheduled_tasks`),
  // so the initial frame batch is the nine queries — NOT a bare git/jobs single.
  const outbound = sock.takeOutbound() as Array<{ collection?: string }>;
  expect(outbound.length).toBe(9);
