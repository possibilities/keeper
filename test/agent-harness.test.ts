...
const SECOND_AXES: ReadonlySet<SecondAxis> = new Set(["effort", "thinking"]);

const HOOK_MECHANISMS = new Set(["claude-hooks", "pi-extension"]);

describe("harness registry", () => {
  test("contains only supported harnesses with complete descriptors", () => {
    expect(HARNESS_NAMES).toEqual(["claude", "pi"]);
...
describe("retired harness config", () => {
  ...
  test("a Codex triple is rejected as an unregistered harness", () => {
    ...
    expect(() => loadPresetCatalog(path)).toThrow(/claude\|pi/);
  });
