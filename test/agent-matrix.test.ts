...
/** A valid active roster: Claude native (opus/sonnet) and Pi serving two
 *  wrapped capabilities, including an opaque provider-qualified launch id. */
const VALID_MATRIX = [
  "efforts:",
  "  - low",
  "  - high",
...
/** An enumeration roster exercising the active Claude/Pi cube: Claude inherits
 *  the top-level axis; Pi carries a provider effort override, an opaque aliased
 *  launch id, and a per-model override. */
const CUBE_MATRIX = [
  "efforts:",
  "  - low",
  "  - high",
...
describe("loadMatrix parsing", () => {
  ...
  test("a valid roster parses with aliases + defaults", () => {
    ...
    expect(m.providers.map((p) => p.name)).toEqual(["claude", "pi"]);
    const pi = m.providers.find((p) => p.name === "pi");
    expect(pi?.models.get("gpt-5.5")).toBe("openai-codex/gpt-5.5");
    expect(pi?.models.get("gpt-5-codex")).toBe("gpt-5-codex");
  });
...
describe("loadMatrix fail-loud shapes", () => {
  ...
  test("a model under claude AND another provider is fail-loud (ambiguous driver)", () => {
    ...
    ).toThrow(/served by both claude and pi/);
  });
  ...
  test.each(["codex", "hermes"])(
    "retired provider %s is fail-loud",
    (provider) => {
      expect(() =>
        loadMatrix(
          writeMatrix(VALID_MATRIX.replace("name: pi", `name: ${provider}`)),
        ),
      ).toThrow(/claude\|pi/);
    },
  );
...
describe("pure derivations", () => {
  ...
  test("providerOrderFor: active wrapped provider, claude excluded", () => {
    const m = validMatrix();
    expect(providerOrderFor(m, "gpt-5.5")).toEqual(["pi"]);
    expect(providerOrderFor(m, "gpt-5-codex")).toEqual(["pi"]);
  ...
  test("nativeIdFor: alias target, else the capability token", () => {
    const m = validMatrix();
    expect(nativeIdFor(m, "pi", "gpt-5.5")).toBe("openai-codex/gpt-5.5");
    expect(nativeIdFor(m, "pi", "gpt-5-codex")).toBe("gpt-5-codex");
    expect(nativeIdFor(m, "claude", "opus")).toBe("opus");
    expect(nativeIdFor(m, "pi", "opus")).toBeNull();
  });
  ...
  test("resolveModel: wrapped model yields the Pi candidate", () => {
...
describe("model long form + legacy alias retirement", () => {
  function withPiModels(...modelLines: string[]): string {
    return [
      "efforts:",
      "  - low",
      "  - high",
  ...
  test("the long form {name} with no native aliases native id === capability", () => {
    const m = loadMatrix(
      writeMatrix(withPiModels("      - name: gpt-5.5")),
    ) as Matrix;
  ...
  test("the retired one-pair alias map is fail-loud, naming the long form", () => {
    expect(() =>
      loadMatrix(
        writeMatrix(withPiModels("      - gpt-5.5: openai-codex/gpt-5.5")),
      ),
  ...
  test("a non-string long-form name is fail-loud (YAML coercion)", () => {
    ...
    expect(() =>
      loadMatrix(writeMatrix(withPiModels("      - name: true"))),
...
describe("per-provider / per-model effort overrides", () => {
  ...
  test("a model with no override inherits the top-level axis", () => {
    const m = matrixWith(["  - name: pi", "    models:", "      - gpt-5.5"]);
...
describe("alias-target charset (slashed native id)", () => {
  // Pi aliases the wrapped capability gpt-5.5 to an opaque provider-qualified
  // launch id, preserving it verbatim through resolution.
  ...
  test("a provider-qualified slashed native id parses as an alias target", () => {
    ...
    expect(nativeIdFor(m, "pi", "gpt-5.5")).toBe("openai/gpt-5.5");
  });
  ...
  test("providers resolve carries the slashed model_id through to the launch flag (pass-through)", async () => {
    // v2 equivalent of SLASHED_TARGET: Pi derives capability gpt-5.5 from the
...
describe("v2 loader — capability derivation + parse", () => {
  ...
  test("matrixV2EffortsFor: per-capability list, else the top-level axis", () => {
    const m = loadMatrixV2(writeMatrix(fx.MULTI_PROVIDER));
    // Spark inherits Pi's provider override; opus the top-level axis; an
...
describe("v2 loader — retired keys and harnesses", () => {
  ...
  test.each(["codex", "hermes"])(
    "rejects retired standalone harness %s",
    (provider) => {
      expect(() =>
        loadMatrixV2(
          writeMatrix(
            fx.MULTI_PROVIDER.replace("name: pi", `name: ${provider}`),
          ),
        ),
      ).toThrow(/claude\|pi/);
    },
...
describe("v2 loader — dedup, shadow, and launch-only enumeration", () => {
  ...
  test("the active two-provider fixture has no shadowed route", () => {
    const m = loadMatrixV2(writeMatrix(fx.MULTI_PROVIDER));
    expect(m.providers.map((p) => p.name)).toEqual(["claude", "pi"]);
  ...
  test("a provider model absent from subagent_models loads as launch-only enumeration", () => {
    ...
    expect(
      m.providers.find((p) => p.name === "pi")?.models.get("gpt-5.5-preview"),
...
describe("committed example matrix (anti-rot, v2)", () => {
  ...
  test("the launcher loader parses the example: bare, {id, efforts}, and provider-qualified forms", () => {
    ...
    expect(m.providers.map((p) => p.name)).toEqual(["claude", "pi"]);
    // Provider-qualified Pi launch id → capability = basename.
    expect(
      m.providers
        .find((p) => p.name === "pi")
        ?.models.get("gpt-5.3-codex-spark"),
...
describe("enumerateTriples (virtual launch cube)", () => {
  test("fans every provider over native ids and effective efforts", () => {
    ...
    // Pi preserves the qualified launch id and effective effort overrides.
  ...
  test("a triple entry carries the capability + native id + effort", () => {
    const cube = enumerateTriples(cubeMatrix());
    const pi = cube.find((g) => g.harness === "pi");
    expect(pi?.triples[0]).toEqual({
      triple: "pi::openai-codex/gpt-5.5::high",
      capability: "gpt-5.5",
      native_id: "openai-codex/gpt-5.5",
      effort: "high",
    });
  });

  test("opaque Pi ids enumerate while capability routing stays basename-keyed", () => {
    const m = cubeMatrix();
    const strings = enumerateTripleStrings(m);
    expect(strings.has("pi::openai-codex/gpt-5.5::high")).toBe(true);
    expect(cellSet(m).map((c) => c.model)).toEqual([
      "opus",
      "sonnet",
      "gpt-5.5",
...
describe("lintHostTriples (host-triple drift + fault)", () => {
  ...
  test("in-cube Claude and Pi triples are clean", () => {
    const findings = lintHostTriples(
      cubeMatrix(),
      hostTripleRefs(
        hostRefs({
          defaults: { pi: "pi::openai-codex/gpt-5.5::high" },
          panels: { duo: ["claude::opus::low", "pi::gpt-fast::low"] },
        }),
      ),
    );
    expect(findings).toEqual([]);
...
describe("extractHostTriples (dual-shape panel harvesting)", () => {
  const members = ["claude::opus::high", "pi::openai-codex/gpt-5.5::high"];
  ...
  test("harvests members from an object-form panel, plus its strength/description", () => {
    ...
    expect(host.panels.duo).toEqual(members);
  ...
  test("harvests members from a legacy list-form panel, with empty strength/description", () => {
    const host = extractHostTriples(null, { panels: { duo: members } });
    expect(host.panels.duo).toEqual(members);
  ...
  test("hostTripleRefs flattens panel-member refs identically over both shapes", () => {
    const listForm = hostTripleRefs(
      extractHostTriples(null, { panels: { duo: members } }),
    );
    ...
    expect(listForm).toEqual([
      ...
      {
        source: "panel 'duo' member 2",
        raw: "pi::openai-codex/gpt-5.5::high",
      },
    ]);
  });
});
...
/** A valid v2 roster for the resolve verb: Claude native (opus/sonnet) and Pi
 *  serving gpt-5.5 through an opaque provider-qualified launch id. */
    ...
    expect(env.candidates).toEqual([
      {
        ...
        preset_name: "pi-gpt-5.3-codex-spark",
      },
    ]);
  });
});
 ...
 *  [low, high]); Pi with a provider effort override [high, xhigh] and a
 *  launch-only capability carrying an opaque slashed launch id. */
  ...
  test("a consistent roster with in-cube host triples is clean (exit 0)", async () => {
    const h = makeHarness({
      ...
      hostTriples: {
        ...
        panels: {
          duo: ["pi::openai-codex/gpt-5.5-codex::high", "claude::opus::high"],
        },
