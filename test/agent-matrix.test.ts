...
describe("matrix provider registry", () => {
  ...
  test("the committed example preserves Pi capability and effort overrides", () => {
    const matrix = loadMatrixV2(
      join(import.meta.dir, "..", "docs", "examples", "matrix.example.yaml"),
    );
    const pi = matrix.providers.find((provider) => provider.name === "pi");
    expect(pi?.models.get("gpt-5.6-sol")).toBe("openai/gpt-5.6-sol");
    expect(pi?.modelEfforts.get("gpt-5.6-sol")).toEqual([
      "medium",
      "high",
      "xhigh",
    ]);
