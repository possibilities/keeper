 ...
 *
 * This worker invokes the REAL scrape entry through the injected runtime (the
 * bun leg for this epic), forced down its fast error arm with
 * `--command /usr/bin/false` — no real claude/codex TUI, no trust mutation — then
 * posts the parsed {@link ScrapeResult} back to the test. The test asserts the
 * contract round-tripped (a real `error` arm, not a `runner_failure` /
...
parentPort?.on("message", async (runtime: UsageScraperRuntime) => {
  try {
