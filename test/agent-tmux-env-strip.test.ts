/**
 * The tmux env strip (src/main.ts): under $TMUX, main() carries the pane id to
 * KEEPER_TMUX_PANE then deletes TMUX/TMUX_PANE from the child env so Claude's
 * ink2 renderer emits truecolor while keeper's hook keeps the pane id for window
 * renaming. Asserted on deps.env post-main() (the mutation Claude inherits),
 * NOT on recorded spawn args (SpawnFn carries only argv).
 */
