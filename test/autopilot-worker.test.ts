...
function makeFakeWorktreeDriver(opts?: {
  ...
  const driver: WorktreeDriver = {
    ...
    async provision(info, liveAttributedDirty) {
      ...
      const pendingIntegration = opts?.provisionPending?.(info) ?? null;
      return pendingIntegration === null
        ? { ok: true, cwd: info.assignment.worktreePath }
        : {
            ok: true,
...
test("28b buildWorktreeStatusEntries: a CLUSTERED epic's reopened-serial DEGRADE surfaces on the projection — absent epoch + prior-lane proof + gained task on an UNSAFE (submodule) repo", () => {
  const epic = makeEpic({
    epic_id: "fn-1-deg",
    project_dir: "/repo-a",
    tasks: [
      ...
      // The GAINED task on the unsafe repo (a refine-apply addition post-teardown).
      makeTask({
        task_id: "fn-1-deg.3",
        task_number: 3,
        target_repo: "/repo-b",
  ...
  const laneProbe = (_e: string, repo: string): LaneProbeResult =>
    repo === "/repo-b"
      ? { epoch: "absent", priorLane: true } // ran a lane before, now torn down
      : { epoch: "present", priorLane: false };
  const map = classifyWorktreeRepos([epic], abResolve, assess, laneProbe, true);
  const entries = buildWorktreeStatusEntries(map);
  expect(entries).toHaveLength(1);
  expect(entries[0]?.epic_id).toBe("fn-1-deg");
  expect(entries[0]?.repo_dir).toBe("/repo-b");
  expect(entries[0]?.mode).toBe("serial");
  expect(entries[0]?.reason).toContain("worktree-reopen-serial");
});

test("28b buildWorktreeStatusEntries: a no-manifest group WITH proof re-cuts to worktree (NO degrade row); an unsafe group with NO gained task / NO proof surfaces NOTHING", () => {
  // (a) no-manifest + absent + proof → RE-CUTS to worktree → not a serial degrade.
  const nm = makeEpic({
    epic_id: "fn-1-nm",
    project_dir: "/repo-a",
    tasks: [
  ...
  const noManifest = (top: string) =>
    top === "/repo-b"
      ? { eligible: false, reason: "worktree-disabled:no-manifest" }
      : { eligible: true, reason: "worktree-eligible" };
  const absentProof = (_e: string, repo: string): LaneProbeResult =>
    repo === "/repo-b"
      ? { epoch: "absent", priorLane: true }
      : { epoch: "present", priorLane: false };
  expect(
    buildWorktreeStatusEntries(
      classifyWorktreeRepos([nm], abResolve, noManifest, absentProof, true),
    ),
  ).toEqual([]);
  // (b) unsafe repo, absent epoch, but NO prior-lane proof → serial WITHOUT a degrade
  //     (never proven to run a worktree lane) → nothing surfaces.
  const submodules = (top: string) =>
    top === "/repo-b"
      ? { eligible: false, reason: "worktree-disabled:submodules" }
      : { eligible: true, reason: "worktree-eligible" };
  const absentNoProof = (_e: string, repo: string): LaneProbeResult =>
    repo === "/repo-b"
      ? { epoch: "absent", priorLane: false }
      : { epoch: "present", priorLane: false };
  expect(
    buildWorktreeStatusEntries(
      classifyWorktreeRepos([nm], abResolve, submodules, absentNoProof, true),
    ),
  ).toEqual([]);
});
...
const RIB3 = "keeper/epic/fn-1-foo--fn-1-foo.3";
const RIB_BASE = "keeper/epic/fn-1-foo";

/** A production-shaped runner over THREE ribs: RIB1 + RIB2 conflict until the
 * out-of-band resolver settles them (added to `resolved`); RIB3 is clean. Ribs merged
 * into the base accumulate in `inBase` so a later pass reads them as ancestors. */
function makeThreeRibAssembleRun(resolved: Set<string>): {
  run: Parameters<typeof createWorktreeDriver>[0];
  cmds: string[];
} {
  const conflicting = new Set([RIB1, RIB2]);
  const cmds: string[] = [];
  ...
  const run: Parameters<typeof createWorktreeDriver>[0] = async (args) => {
    ...
    if (joined.startsWith("merge-base --is-ancestor")) {
      ...
      const isIn = inBase.has(src) || resolved.has(src);
    ...
    if (args[0] === "merge") {
      ...
      if (conflicting.has(src) && !resolved.has(src)) {
        midMerge = true;
        return {
          code: 1,
          stdout: "",
...
function makeThreeRibSinkInfo(): WorktreeLaunchInfo {
  return {
    assignment: {
    ...
    laneOrder: [
      ...
      { nodeId: "fn-1-foo.3", branch: RIB3, worktreePath: "/w3" },
...
test("fn-16f runReconcileCycle: an incident-owner close whose fan-in DEFERS (transient) is SUPPRESSED — no launch (no attachment burned on a non-conflict), no sticky", async () => {
  const { driver } = makeFakeWorktreeDriver({
    assembleDefer: (info) =>
      info.repoDir === "/repo-a"
        ? "worktree-lane-premerge-not-ready: base not quiescent"
        : null,
  });
  const { deps, log: depsLog } = makeFakeDeps({ worktree: driver });
  await runReconcileCycle(
  ...
  // A transient defer is NOT a positively-current conflict → the resolver launch is
  // suppressed even for the incident owner (no attachment consumed)...
  expect(
    depsLog.launches.find((l) => l.name === "close::fn-1-anchor"),
  ).toBeUndefined();
  // ...and mints NO sticky.
...
test("fn-16f runReconcileCycle: an incident-owner close with a STRUCTURAL failure mints a DURABLE fence on a DISTINCT per-repo key (never the bare incident key) and is SUPPRESSED", async () => {
  const { driver } = makeFakeWorktreeDriver({
    assembleFail: (info) =>
      info.repoDir === "/repo-a"
  ...
  // Suppressed (a structural failure blocks the launch, never a console-only log)...
  expect(
    depsLog.launches.find((l) => l.name === "close::fn-1-anchor"),
  ).toBeUndefined();
  // ...the fence rides a DISTINCT worktree-preclose:<epic>-<hash> key...
  const fence = depsLog.emissions.find(
    (e) =>
      e.verb === "close" && e.id.startsWith("worktree-preclose:fn-1-anchor-"),
  );
  expect(fence).toBeDefined();
  expect(fence?.reason).toContain("worktree-head-mismatch");
  // ...and NEVER masquerades as / clobbers the bare close::<epic> incident key.
  expect(
...
test("fn-16f runReconcileCycle: a conflict in one worktree group does NOT stop assembling a LATER clean group — every group is fanned in, exactly one incident routed", async () => {
  ...
  // BOTH groups were assembled — the later /repo-b was NOT short-circuited by /repo-a's
  // conflict (its clean rib would otherwise strand and wedge the ancestry gate).
  expect(log.provisions.map((i) => i.repoDir).sort()).toEqual([
    "/repo-a",
    "/repo-b",
  ]);
  // Exactly ONE incident routed (the first conflict); the non-owner close is suppressed.
  const incidents = depsLog.emissions.filter(
    (e) => e.verb === "close" && e.id === "fn-1-anchor",
  );
...
test("runReconcileCycle: pending fan-in is recorded before the owning work launch", async () => {
  const laneDir = worktreePathFor("/home/me/repo", "keeper/epic/fn-1-foo");
  const sourceBranch = "keeper/epic/fn-1-foo--fn-1-foo.2";
  const { driver } = makeFakeWorktreeDriver({
    provisionPending: () => ({
      sourceBranch,
  ...
  expect(log.dispatchedEmissions).toHaveLength(1);
  expect(log.launches).toHaveLength(1);
  expect(log.launches[0]?.cwd).toBe(laneDir);
});
...
test("createWorktreeDriver: finalize defers while a provisioned rib is not integrated into the base", async () => {
  const base = "keeper/epic/fn-1-foo";
  const rib = `${base}--fn-1-foo.2`;
  const cmds: string[] = [];
  const fakeRun: Parameters<typeof createWorktreeDriver>[0] = async (args) => {
    const joined = args.join(" ");
    cmds.push(joined);
    if (joined === `rev-parse --verify --quiet refs/heads/${base}`) {
      return { code: 0, stdout: "base\n", stderr: "" };
    }
    if (joined.startsWith("symbolic-ref")) {
      return { code: 0, stdout: "origin/main\n", stderr: "" };
    }
    if (joined === `rev-parse --verify --quiet refs/heads/${rib}`) {
      return { code: 0, stdout: "rib\n", stderr: "" };
    }
    if (joined === `merge-base --is-ancestor ${rib} ${base}`) {
      return { code: 1, stdout: "", stderr: "" };
    }
    return { code: 0, stdout: "", stderr: "" };
  };
  ...
  const info: WorktreeLaunchInfo = {
    assignment: {
      ...
      isCloseSink: true,
      branch: base,
      worktreePath: "/repo.worktrees/base",
      inherited: true,
      preMerges: [],
      assertBranch: base,
    },
    baseBranch: base,
    baseWorktreePath: "/repo.worktrees/base",
    repoDir: "/repo",
    laneOrder: [
      {
        nodeId: "fn-1-foo.1",
      ...
      },
      {
        nodeId: "fn-1-foo.2",
        branch: rib,
        worktreePath: "/repo.worktrees/rib",
      },
    ],
    parentBranch: base,
  };

  const result = await driver.finalizeEpic(info, async () => true);

  expect(result).toEqual({
    ok: false,
    retry: true,
    reason: `worktree-finalize-pending-integration: ${rib} is not an ancestor of ${base}`,
  });
  expect(cmds.some((cmd) => cmd.startsWith("update-ref"))).toBe(false);
  expect(cmds.some((cmd) => cmd.startsWith("push"))).toBe(false);
  expect(cmds.some((cmd) => cmd.startsWith("worktree remove"))).toBe(false);
});
...
test("fn-959 createWorktreeDriver: provision forks the BASE lane off the resolved default branch, not its own (uncreated) branch", async () => {
  ...
  const fakeRun: Parameters<typeof createWorktreeDriver>[0] = async (args) => {
    ...
    if (joined.startsWith("worktree list")) {
      // git ALWAYS lists at least the main worktree — an empty list is UNKNOWN.
      const main = "worktree /repo\nHEAD main000\nbranch refs/heads/main\n\n";
      return added
        ? {
            code: 0,
...
// ---------------------------------------------------------------------------
// Fan-in provision records an owner integration manifest without merging.
// ---------------------------------------------------------------------------
...
test("createWorktreeDriver: provision leaves a real source unmerged and returns its manifest after a phantom", async () => {
  const phantom = "keeper/epic/fn-1-foo--fn-1-foo.2";
  const source = "keeper/epic/fn-1-foo--fn-1-foo.3";
  const { run, cmds } = makePhantomFanInRun({ phantomSources: [phantom] });
  const driver = createWorktreeDriver(run);
  const res = await driver.provision(makeFanInInfo([phantom, source]), null);
  ...
  expect(cmds).not.toContain(`merge --no-edit ${source}`);
  expect(cmds.some((c) => c.startsWith("merge --abort"))).toBe(false);
});

test("createWorktreeDriver: provision prepares only the first unresolved source in one dispatch", async () => {
  const source = "keeper/epic/fn-1-foo--fn-1-foo.2";
  const later = "keeper/epic/fn-1-foo--fn-1-foo.3";
  const { run, cmds } = makePhantomFanInRun({ phantomSources: [] });
  const driver = createWorktreeDriver(run);
  const res = await driver.provision(makeFanInInfo([source, later]), null);
...
test("createWorktreeDriver: an inconclusive ancestry probe never mints a pending integration manifest", async () => {
  const source = "keeper/epic/fn-1-foo--fn-1-foo.2";
...
test("fn-16e createWorktreeDriver: assembleBase DEFERS a transient worktree-add timeout (124) in ensure — no permanent structural sticky", async () => {
  ...
  const run: Parameters<typeof createWorktreeDriver>[0] = async (args) => {
    ...
    if (joined.startsWith("worktree list")) {
      // Only the main worktree — the base is not yet added, so ensure runs `add`.
      return {
        code: 0,
...
// makes the scratch "checkout" a real tmp directory (so `readTestGateCommand`
...
test("fn-1213 readTestGateCommand: reads the exact named script and returns null for malformed or absent scripts", () => {
  const dir = mkdtempSync(join(tmpdir(), "keeper-mg-pkgcmd-"));
  const malformedDir = mkdtempSync(join(tmpdir(), "keeper-mg-pkgcmd-bad-"));
  const missingDir = mkdtempSync(join(tmpdir(), "keeper-mg-pkgcmd-missing-"));
  const command = "bun scripts/test-gate.ts --timeout=30000 ./test";
  writeFileSync(
  ...
  writeFileSync(
    join(malformedDir, "package.json"),
    JSON.stringify({ scripts: { "test:gate": 42 } }),
  );
  expect(readTestGateCommand(dir)).toBe(command);
  expect(readTestGateCommand(malformedDir)).toBeNull();
  expect(readTestGateCommand(missingDir)).toBeNull();
...
test("fn-1213 runPackageSuiteGate: a clean gate with a performance-budget warning stays green", async () => {
  ...
  const { spawnFn } = makeQueuedSpawn([
    resolved(0), // install
    resolved(0, "warning: performance budget exceeded\n1 pass\n0 fail\n"),
  ]);
  const verdict = await runPackageSuiteGate(pkgDir, {
    installTimeoutMs: 2_000,
    suiteDeadlineMs: 2_000,
