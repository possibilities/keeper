...
const LANE_GLYPH = "⑂";
const SEP = "∕";
const NF0_FILLED = "\u{f03a1}";
const NF1_FILLED = "\u{f03a4}";
const NETWORK_GLYPH = "";
...
describe("statusline render", () => {
  test("uses the soft slash separator and renders version/profile last", () => {
    ...
    expect(plain).toBe(
      `13 ${SEP} main ${SEP} jobsearch ${SEP} opus 4.8 ${SEP} xhigh ${SEP} 2.1.204 ${NF0_FILLED}`,
    );
  ...
  test("network and non-default profile ride in the final version segment", () => {
    ...
    expect(plain.endsWith(`2.1.204 ${NETWORK_GLYPH} ${NF1_FILLED}`)).toBe(true);
  });
