...
## Done summary
Deleter identified as a ONE-TIME incomplete `git worktree` (re-)provision checkout of the done multi-repo epic fn-688's keeper-side lane: git wrote the index but was cut off before materializing 393 working-tree files, leaving them as ` D` deletions (no untracked/modified; changing file set = interrupted at a different point each time). Proven not an active deleter: `git checkout -- .` fully restored every file from the intact index, and the recover sweep tore the lane+branch down within 15s and it stayed gone. Recover sticky row auto-cleared on teardown; finalize row + fn-884 row dropped via retry_dispatch (fn-884 after removing its orphaned, git-untracked docs lane dir whose 94 files were byte-identical to docs main). All three rows stayed cleared over a ~20min soak with zero re-mint. fn-884 cause: a done multi-repo epic's per-repo docs finalize re-provisions a lane whose admin was pruned but dir orphaned, so `git worktree add` collides ('already exists') and re-mints each cycle. Source fix: spawnGitExec now converts a vanished-cwd posix_spawn ENOENT throw into a GIT_SPAWN_FAILED_CODE(127) result so classifyLinkedWorktree DEFERS instead of the whole recover sweep aborting (the log showed this throw 42x) — the durable reason dead lanes lingered unreaped.
## Evidence
