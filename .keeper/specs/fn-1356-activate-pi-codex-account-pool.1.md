...
**YOUR LANE WORKTREE IS STALE — read this before any source verification.** The fn-1356 epic lane predates fn-1378 (`c1f897457`), the epic that landed the ENTIRE proof-producer: `codex_pool_proof` registration, the proof-window arming flag, and the install.md proof ritual all exist on the primary checkout `/Users/mike/code/keeper` (= what the installed `keeper` on PATH executes) and are ABSENT from your lane's tree. Every source citation below refers to the PRIMARY checkout. Do not verify producer claims against your lane worktree — attempts #11 and #12 both burned exactly there, concluding "no producer" from an honest read of the stale lane. Your task consumes the INSTALLED runtime and writes only proof/doc artifacts (see Files), so the lane's staleness is irrelevant to your work; lane-vs-main reconciliation is the close machinery's job, not yours.

**The proof-producer entry point, verified in source on the primary checkout (do not park on "no producer"):** the window is armed by the LAUNCHER flag on the managed pi launch itself — the literal ritual is documented at docs/install.md:228 ("enroll → arm a proof window → prove → activate") and docs/install.md:245-250 (`keeper agent run pi --x-codex-pool-proof-window=arm ...` — the probe directs Pi to invoke `codex_pool_proof` exactly once). The chain in source: src/agent/main.ts:909 defines `--x-codex-pool-proof-window=arm` (fresh managed Pi sessions only, main.ts:3546), main.ts:3919-3920 sets `KEEPER_PI_CODEX_POOL_PROOF_WINDOW` in the leg env under codexMode "proof", and integrations/pi-codex-pool/src/index.ts:673 consumes it, gating the pi.registerTool of `codex_pool_proof` at index.ts:1165. The `keeper agent accounts codex-pool` surface deliberately carries only CONSUMER verbs (status/verify/rollback/proof capture/proof verdict/activate) — the absence of an arming verb there is by design, not a missing producer.
