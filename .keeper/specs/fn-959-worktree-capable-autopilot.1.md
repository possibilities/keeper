...
guard. SURFACE THE LIVE SETTING ON `keeper autopilot`: add a `worktree`
segment to the banner (`autopilotBannerLabel`) alongside paused/mode/cap, and
include it in any status/`show` output — the no-arg viewer subscribes to
`autopilot_state`, so the segment updates live with no extra wiring. The
guard: enabling/disabling `worktree_mode` is rejected (loud) when ANY epic is
mid-flight (live dispatched/running job or an existing `keeper/epic/*`
worktree); `--force` bypasses. Bump `SCHEMA_VERSION` and add the new version
to `SUPPORTED_SCHEMA_VERSIONS` in `keeper/api.py` in the SAME commit. Fold must
UPSERT preserving sibling columns (paused/mode/cap).
...
- cli/autopilot.ts:817-832 (mode subcommand dispatch), :453-470 (buildSetModeFrame), :312 (projectAutopilotMode), :510-535 (autopilotBannerLabel) — clone for the worktree subcommand AND the banner segment; :854 subcommand error string.
...
- Banner real estate: keep the worktree segment terse (e.g. only render `· worktree` when ON, or `· worktree:off`) so the banner stays scannable; coordinate format with fn-953/fn-954 which also touch this banner.

### Test notes

Fast-tier reducer/CLI unit tests: fold preserves siblings; fresh-DB default
OFF; replay reproduces a set value; `autopilotBannerLabel` renders the
worktree segment for BOTH on and off. schema-version test stays green.
...
- [ ] `keeper autopilot worktree <on|off> [--force]` works.
- [ ] `keeper autopilot` shows the live `worktree_mode` state: a dedicated segment in `autopilotBannerLabel` alongside paused/mode/cap that updates live off the `autopilot_state` subscription, AND any status/`show` output includes it.
