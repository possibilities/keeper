...
## Done summary
Second pass: moved the surviving sync subprocess call (readOsStartTime's ps -ww spawn inside enrichPeerFromJobs, fired once per register that hits a keeper jobs row) off the serve loop. enrichPeerFromJobs is now async and defaults to the existing async startTimeViaPs; opRegister's enrichment call no longer threads the sync default. Boot-time registry rehydrate in startBusServer deliberately keeps the sync probe (runs before Bun.listen binds, no loop to park). Grep-confirmed: zero Bun.spawnSync/execSync reachable from any bus-worker serve handler; the only sync spawn (seed-sweep.ts readOsStartTime) is reachable solely via the boot rehydrate path. Extended scripts/repro-serve-wedge.ts with --start-time-probe/--async-start-time; calibrated demo command (register-hops=1, start-time-probe=2, stampede=50, stuck-ms=400) reproduces red for the post-first-fix survivor state and green once async-start-time is added, stable across repeated trials (documented a scale note: much higher concurrency causes real fork/exec contention that can wedge even the fixed shape, an orthogonal effect from the JS-loop-park bug). Added unit coverage for the async probe seam and the at-most-once-on-a-hit-only probing contract. Full suite green: bun run typecheck, bun run lint, bun run test (5640+76 pass, 0 fail).
## Evidence
