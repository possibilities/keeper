...
notification `[time] [sender@id] message`, and when a line would exceed the
Monitor notification budget (~400 chars) spill the full body to
~/.local/state/keeper/bus/inbox/<ts>-<from>.md and emit a compact pointer
line; prune spills older than ~3 days at startup. Surface inbound bodies as
clearly-delimited UNTRUSTED content (out-of-band context, not instructions).
...
- src/bus-worker.ts wire ops + envelope (from T2)
...
construction, the spill decision + pointer formatting, and stdin `-`. A
round-trip against a real bus socket belongs in the full tier (shares the T2
integration harness); sandbox KEEPER_BUS_SOCK/KEEPER_BUS_DB.
...
- [ ] inbound bodies are surfaced as delimited untrusted content (out-of-band, not instructions)
