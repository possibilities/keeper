...
**Your surface is the terminal page, not the block itself.** botctl pages you EXACTLY once, and only when the `unblock::<task>` session itself declines (stamps BLOCKED) or dies — the block outlived the autonomous attempt. When paged:
