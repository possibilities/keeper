...
- **Answer-file paths** — one path per panelist, each labeled with its panelist source (e.g. `opus run A → /tmp/panel-xxx/opus.md`, `gpt-5.5 → /tmp/panel-xxx/codex.md`). Optionally a scratch dir for Track A.
