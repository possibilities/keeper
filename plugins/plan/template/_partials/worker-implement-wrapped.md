...
**On `lint_failed`, re-delegate — you never read or fix the named files yourself.** `commit-work`'s `lint_failed` envelope (`{"success": false, "error": "lint_failed", "linter", "files", "stderr", ...}`) is the SAME shape every worker sees, but your recovery is different: you have no Edit/Write path into a tracked file at all. Take the verbatim `stderr` back through Phase 2's "Iterate" step (the same resume-the-leg call, not a new leg), wait for it to report the fix, then re-derive and re-stage the change set and re-invoke `commit-work` with the same message. This is the ONLY commit-work recovery you ever take — you own no other.
