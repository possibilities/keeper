...
`/plan:hack` (`skills/hack/SKILL.md`) is a hand-authored STATIC skill — no `.tmpl`, no `.managed-file-dont-edit` sidecar — that investigates a request, answers in the right shape, and routes or executes the next move. It is slash-only (`disable-model-invocation: true`). It bakes three shared promptctl snippets (`keeper-history-forensics` inlined; `escalate-inline-or-plan` and `commit-via-keeper-default` baked verbatim), each under a `Canonical source: promptctl render engineering/<name>` cite line that is the only drift guard.
