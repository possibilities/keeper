...
`assign-cells <epic_id> --file -` and `apply-selection <epic_id> --file -` share one apply core — flock the epic, mutate every named task's `{tier, model}` under it, write the schema-versioned sidecar to `.keeper/selections/<epic_id>.json`, re-validate the tree through the post-write integrity gate (the marker is left untouched, so a ghost stays a ghost until the trailing `validate --epic` arm), then auto-commit the cell writes AND the sidecar in ONE commit — no partial writes on any failure. There is no single-task form of either, which is what keeps them outside the removed incremental `task set-tier` class. They are the two entry points onto that shared core: `assign-cells` takes a hand- or tooling-authored YAML batch — a `cells:` list covering EVERY todo task of the epic exactly once (choosing the default is an explicit cell) plus a `selection:` provenance block (selector source, config + input hashes, shuffle seed, outcome, raw verdict) — and asserts the whole batch itself (assert-all, collect-all); `apply-selection` (below) takes the raw stdin JSON a `plan:model-selector` subagent returned and validates it against the on-disk selection brief instead of trusting the caller. Their callers are the `/plan:plan` (Phase 6.5), `/plan:defer` (Phase 4b), and `/plan:close` (Phase 3.5, follow-up) select beats: each pipes the content-blind `plan:model-selector` subagent's raw return straight to `apply-selection`, landing cells with `label_source: heuristic-guided`, and every selector failure degrades to the stamped defaults recorded with `label_source: heuristic-default`. Manual/degraded/legacy paths store nullable Spark-fit evidence as `null` rather than fabricating selector proof. Axis validation comes from the required effective host matrix. Typed errors (`assign-cells`): `bad_yaml` (shape/type), `cell_invalid` (out-of-axis tier/model, or an unknown / duplicate / missing / non-todo task id — the full-set + todo-only contract), `epic_not_found`.

`apply-selection <epic_id> --file -` (ADR 0027) is the ONE trusted apply seam for a model-selector verdict: the calling skill pipes the `plan:model-selector` subagent's raw JSON return verbatim on stdin (a single ```` ```json ```` fence is tolerated) — never parses, enum-clamps, or transcribes it — and the verb parses, schema-validates, enum-clamps each `tier`/`model` against the candidate axes, asserts exact coverage of the on-disk selection brief (`.keeper/state/selections/<epic_id>/brief.json`, written by `selection-brief`), and validates the exact Spark evidence shape before landing anything. The selector must return `spark_fit` and `spark_exclusion` for every cell: Spark selections require `spark_fit: true` plus `spark_exclusion: null`; non-Spark selections require `spark_fit: false` plus one closed reason, with `spark-not-on-axis` allowed only when Spark is absent from the brief's model axis. Provenance (`config_hash`, `input_hash`, `shuffle_seed`) is pinned from that brief, never transcribed from the model's own claims. On the live path it lands cells through the shared core above with `label_source: heuristic-guided`. `--from-followup` re-targets validation at `followup-brief.json` and, instead of a live write, atomically stages the verdict as a gitignored `followup-verdict.json` sibling of the brief — the document `close-finalize --selection-verdict` revalidates when a follow-up epic's tasks are born pre-selected — returning `verdict_path` rather than landing a commit; `/plan:close` (Phase 3.5) is its caller. `--degraded <reason>` (live-only, rejected together with `--from-followup`) skips validation entirely and re-asserts each current todo task's own stamped cell with `label_source: heuristic-default` and null Spark evidence, so a selector failure still leaves the epic armable — every calling skill's two-strike contract (one relayed retry, then degrade) bottoms out here. Typed errors: `verdict_invalid` (bad JSON, bad shape, inconsistent Spark evidence, an error-shaped return, an unknown top-level key, an enum-clamp miss, or a coverage miss against the brief), `brief_missing` (no brief on disk for this epic/mode, or a `from_followup` mismatch), `cell_invalid` (from the shared core — out-of-axis tier/model, or an unknown / duplicate / missing / non-todo task id), `epic_not_found`.
...
{project_root}/.keeper/
  ...
  specs/{task-id}.md
  tasks/{task-id}.json
  selections/{epic-id}.json # committed model+effort selection sidecars (apply-selection/assign-cells)
...
The post-scaffold **model selector** (`plan:model-selector`, fed by `keeper plan selection-brief`) assigns each todo task's `{tier, model}` cell — wrapped capability models included — from the configured axes, batch-written by `apply-selection`. `/plan:model-guidance` authors the per-axis-value guidance the selector reads; every roster model needs a guidance block, enforced fail-loud by the drift gate.

On the model axis the selector's binding tie-break is a human-owned, hand-tuned policy — `model-selector.yaml`'s `hand_tuned` block, carried verbatim in every selector brief and never touched by `/plan:model-guidance`'s research pass (which only refreshes the guidance around it). The burden of proof is on opus: choose it only for a concrete, nameable intelligence-bound reason, including a task that must derive, adjudicate, or validate a subtle correctness/security invariant; mechanical implementation or deterministic verification of an already-specified invariant follows ordinary capability routing. Spark fits only small, reversible, fixed-shape edits across a few explicitly named files (including routine navigation and deterministic verification) and is excluded by open-ended diagnosis, cross-module discovery/reasoning, contract or security design, ambiguity/taste/judgment, subtle invariant reasoning, and long trajectories/context needs. Mechanical or templated work biases the effort axis toward the lower bands. Edit `hand_tuned` directly to retune the policy — it is cheap-to-change config (ADR 0010), not code.
