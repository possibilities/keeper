...
| `/plan:prompt <prompt>` | Polish a raw prompt into a sharper one for the agent that will read it — one human-approved change per turn toward an announced word-count target (headline/blurb/note/memo/brief/spec), with a constraint-polarity check on every rewrite, until ready. Read-only wrt the repo, clipboard-only export, nothing written to disk. Slash-only (`disable-model-invocation: true`). Hand-authored static skill — no template, no `.managed-file-dont-edit` sidecar. |
