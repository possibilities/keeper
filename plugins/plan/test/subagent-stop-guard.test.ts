...
// letting us assert "zero planctl subprocesses" for the short-circuits.
...
describe("VERDICT_NUDGE", () => {
  test("in_progress_committed names the task and points at done", () => {
    ...
    expect(nudge).toContain("planctl done");
  });
...
describe("taskIdFromTranscript", () => {
  let dir: string;
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "planctl-transcript-"));
  });
  afterEach(() => {
...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), "planctl-subagent-guard-"));
...
describe("subagent-stop-guard ladder", () => {
  ...
  test("blocks on in_progress_committed pointing at planctl done", async () => {
    writeWorkMarker("fn-1-x.2");
    ...
    const env = JSON.parse(stdout.trim());
    expect(env.decision).toBe("block");
    expect(env.reason).toContain("planctl done fn-1-x.2");
  });

  test("blocks on state_uncommitted", async () => {
    writeWorkMarker("fn-1-x.2");
    writePlanctlShim({ verdict: "state_uncommitted", task_id: "fn-1-x.2" });
  ...
  test("not_started verdict allows — never trap an unstarted task", async () => {
    writeWorkMarker("fn-1-x.2");
    writePlanctlShim({ verdict: "not_started", task_id: "fn-1-x.2" });
