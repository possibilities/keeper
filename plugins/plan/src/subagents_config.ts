...
//
// The embedded/disk subagents.yaml above is the claude-only DEFAULT matrix. A
// host-level `~/.config/keeper/matrix.yaml`, when present, OVERRIDES the axes at
// run time: an ordered provider roster grows the model axis with capability
// models served by non-claude harnesses. `effectiveMatrix()` composes the two —
// host when present, embedded defaults otherwise — so both consumers (the plan
// verbs at runtime, the prompt renderer at build time) see one effective matrix.
//
