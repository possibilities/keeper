 ...
 * on the board that a claude session does.
 *
 ...
 * or interfere with the human's pi session. Handlers are pure observers: they
 * return nothing, so they never block a tool call or rewrite a message.
 *
 ...
 * uuid == `jobs.job_id`). This extension owns LIVE STATE only, so it emits no
 ...
 * lifecycle into the events-log. NO-OPS entirely when `KEEPER_JOB_ID` is absent
