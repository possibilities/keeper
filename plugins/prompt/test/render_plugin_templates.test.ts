  ...
  test("renders the wrapped delegate contract and keeps the native implement phase", () => {
    ...
    try {
      ...
      for (const required of [
        ...
        "attacker-influenced",
        "require the authoritative test pass",
        "never execute repository code in the write-denied wrapper",
        "Raw `git reset` is denied",
        "private system-temp directory",
        "ATOMIC_HANDOFF_WRITTEN",
        "core.fsmonitor=false",
        "provider-supplied `Task:`",
        "--task-id <literal-task-id>",
        "Task: $TASK_ID",
        "Job-Id:",
        "keeper commit-work",
