...
fn-587 task .4: this module was renamed from ``validation_clear.py`` and
``VALIDATION_CLEAR_VERBS`` was renamed to ``VALIDATION_RESTAMP_VERBS``.  The
list contents are unchanged (same 13+1 verbs) — only the semantic flipped
from "clear the marker on mutation" to "re-stamp the marker on mutation
after a post-write integrity re-check."  ``epic invalidate`` remains the
...
#
# fn-565 removed the incremental `task-create` / `set-spec` / `set-deps` /
# `dep-add` / `set-plan` entries alongside their CLI verbs — the structural
# changes those verbs made now ride `scaffold` (mint; NOT in this tuple) and
# `refine-apply` (rewrite an existing tree; IS in this tuple).
VALIDATION_RESTAMP_VERBS: tuple[str, ...] = (
    "set-description",
    "set-acceptance",
    "reset",
    "add-dep",
    # fn-565: batch epic-dep wirer — same structural effect as add-dep (wires
    ...
    "set-touched-repos",
    "set-target-repo",
    # fn-513: spec-metadata setters — replacing the snippet/bundle list is a
    # structural spec change (mirrors set-deps). Shared verb name across the
    # task and epic surfaces; both re-stamp the marker.
    "set-snippets",
    "set-bundles",
    # fn-565: refine-apply rewrites specs/deps on an EXISTING epic tree (adds,
...
def restamp_epic_or_fail(
    ...
    *,
    ...
    ``check_filesystem_repos`` (fn-589 task .1, item 2): when True, the
    ...
    # helper's epic-dep cycle check (fn-588 task .1).  Single glob covers
    ...
    # fn-600: extend existence + cycle universe across every discovered
