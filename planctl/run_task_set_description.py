"""planctl task set-description - Set the Description section of a task spec."""
...
def _render_human(data: dict) -> str:
    return f"Updated Description for {data.get('task_id')}"
...
def run(args: SimpleNamespace) -> int:
    ...
    # Patch section
    try:
        patched = patch_task_section(current_spec, "## Description", new_content)
    ...
    new_stamp = restamp_epic_or_fail(epic_id, data_dir, verb="set-description")
