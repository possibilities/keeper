"""planctl epic set-epic-review-status - Set the epic review status on an epic."""
...
def _render_human(data: dict) -> str:
    return (
        f"Set epic review status for {data.get('epic_id')}: "
        f"{data.get('epic_review_status')}"
    )
...
def run(args: SimpleNamespace) -> int:
    ...
    reviewed_at = now_iso()
    epic_def["epic_review_status"] = status
    epic_def["epic_reviewed_at"] = reviewed_at
    ...
    emit(
        {
            "epic_id": epic_id,
            "epic_review_status": status,
            "epic_reviewed_at": reviewed_at,
        },
        text_renderer=_render_human,
        verb="set-epic-review-status",
        target=epic_id,
        detail=status,
