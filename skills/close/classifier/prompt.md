...
This document describes the prompt contract the closer skill uses when spawning the `classifier` agent. The closer constructs the actual message by interpolating `EPIC_ID`, `PRIMARY_REPO`, and the auditor report — this file is the canonical reference for the structure and conventions.
...
The classifier opens as **an impartial expert advisor**, not a "helpful reviewer." It holds ground on judgement calls. It does not flag findings to be helpful. The bar is high: flag only what has real user impact. **Leaving code alone is the default.**

The same critical directive appears at both the START and END of the classifier system prompt (U-shaped attention discipline) so it stays salient across both prose authoring and the final verdict emit.
...
| `tier_3` | Large cross-cutting followup candidate | No closer action; surfaces in `/plan:audit` |
...
When in doubt, NOT fatal — route to `tier_1` / `tier_2` / `tier_3` and let `/plan:audit` plan the followup.

## Filtering Discipline (mirrored verbatim across both files)
...
The closer passes this as the user message to the classifier agent. The classifier's system prompt (in `apps/planctl/agents/classifier.md`) provides all behavioral instructions.
