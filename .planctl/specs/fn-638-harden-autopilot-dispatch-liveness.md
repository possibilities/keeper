...
- `fn-636-add-board-pill-coverage-for-cross` (overlap, not a dependency) — its sole task is already done; both it and task `.4` touch `scripts/board.ts` `renderEpicBlock`/`colorizePillsInLine`. Not hard-wired as an epic dep (that would gate the core autopilot/reducer work); task `.4` carries a sequencing note instead.
...
- **CLAUDE.md** event-sourcing invariants fan-out enumeration — name the new bounded-Stop `state` transition if it fans through `syncJobLinksOnJobWrite`.
