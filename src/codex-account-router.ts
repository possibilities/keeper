...
function parsePressureEntry(value: unknown): PressureEntry | null {
  ...
  const cooldown = timestamp(input.cooldown_until_ms);
  const last =
  ...
  return {
    reservations: reservations as number[],
...
function prunePressureLedger(
  ...
  for (const observed of observation.aliases) {
    ...
    if (!current) continue;
    const cooldownValid =
      current.cooldown_until_ms > nowMs &&
      current.cooldown_until_ms <= nowMs + CODEX_FAILURE_COOLDOWN_MS;
    if (current.cooldown_until_ms > 0 && current.cooldown_until_ms <= nowMs) {
    ...
    aliases[observed.alias] = {
      ...
      cooldown_until_ms: cooldownValid ? current.cooldown_until_ms : 0,
      last_selected_at_ms:
...
function effectiveUsedPercent(
  window: CodexCapacityAlias["windows"][number],
  nowMs: number,
): number {
...
function worstUsedPercent(alias: CodexCapacityAlias, nowMs: number): number {
  return Math.max(
    0,
    ...alias.windows.map((window) => effectiveUsedPercent(window, nowMs)),
  );
}

function usableAliases(
  observation: CodexCapacityObservation,
  nowMs: number,
): CodexCapacityAlias[] {
  return observation.aliases.filter(
    (alias) =>
      alias.status === "healthy" &&
      isCodexAliasFresh(alias, nowMs) &&
      alias.windows.length > 0 &&
      worstUsedPercent(alias, nowMs) < 100,
  );
}

function entryFor(ledger: PressureLedger, alias: string): PressureEntry {
  return (
    ledger.aliases[alias] ?? {
      reservations: [],
      cooldown_until_ms: 0,
...
function compareCandidates(
  left: CodexCapacityAlias,
  right: CodexCapacityAlias,
  ledger: PressureLedger,
  nowMs: number,
): number {
  const leftEntry = entryFor(ledger, left.alias);
  const rightEntry = entryFor(ledger, right.alias);
  const leftScore =
    worstUsedPercent(left, nowMs) +
    leftEntry.reservations.length * CODEX_PRESSURE_PERCENT_STEP;
  const rightScore =
    worstUsedPercent(right, nowMs) +
    rightEntry.reservations.length * CODEX_PRESSURE_PERCENT_STEP;
  if (leftScore !== rightScore) return leftScore - rightScore;
  const leftLast = leftEntry.last_selected_at_ms ?? Number.NEGATIVE_INFINITY;
  const rightLast = rightEntry.last_selected_at_ms ?? Number.NEGATIVE_INFINITY;
...
export function selectCodexRoute(
  ...
  try {
    ...
    try {
      ...
      const candidates = usableAliases(observation, nowMs);
      if (candidates.length === 0) return fallback("pool-unavailable");
      ...
      const eligible = candidates.filter(
        (alias) =>
          entryFor(pruned.ledger, alias.alias).cooldown_until_ms <= nowMs,
      );
      if (eligible.length === 0) return fallback("pool-unavailable");
      const selected = [...eligible].sort((left, right) =>
        compareCandidates(left, right, pruned.ledger, nowMs),
      )[0];
      const entry = entryFor(pruned.ledger, selected.alias);
      entry.reservations = [...entry.reservations, nowMs].slice(
        -CODEX_MAX_RESERVATIONS_PER_ALIAS,
      );
      entry.last_selected_at_ms = nowMs;
      pruned.ledger.aliases[selected.alias] = entry;
      writePressureLedger(stateDir, pruned.ledger);
      return {
        kind: "pooled",
        provider: CODEX_PROVIDER,
        alias: selected.alias,
        reason: pruned.recovered.has(selected.alias)
          ? "cooldown-recovered"
          : eligible.length === 1
            ? "sole-candidate"
            : "selected",
  ...
  try {
    ...
    try {
      ...
      entry.cooldown_until_ms =
        deps.outcome === "success" ? 0 : nowMs + CODEX_FAILURE_COOLDOWN_MS;
...
export function inspectCodexRouting(
  ...
  try {
    ...
    const usable = new Set(
      usableAliases(observation, nowMs).map((alias) => alias.alias),
    );
    const candidates = observation.aliases.map((alias) => {
      const entry = entryFor(pruned, alias.alias);
      return {
        alias: alias.alias,
        worst_used_percent: worstUsedPercent(alias, nowMs),
        pressure: entry.reservations.length,
        cooldown_until_ms: entry.cooldown_until_ms,
        eligible: usable.has(alias.alias) && entry.cooldown_until_ms <= nowMs,
      };
    });
    ...
    if (available.length === 0) {
      ...
      const byAlias = new Map(
        observation.aliases.map((alias) => [alias.alias, alias]),
      );
      const selected = [...available]
        .map((candidate) => byAlias.get(candidate.alias) as CodexCapacityAlias)
        .sort((left, right) =>
          compareCandidates(left, right, pruned, nowMs),
        )[0];
      verdict = {
        kind: "pooled",
        provider: CODEX_PROVIDER,
