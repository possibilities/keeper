 ...
 *     = winner's `title` verbatim (a spawn-name title like `work::fn-1019.2`
 *     tabs as `work::fn-1019.2`); emit a rename ONLY
 ...
 * joined `(pane_id, title, created_at, state)`-equivalent tuple. An empty set
 * is the empty-string hash. Mirrors restore-worker's `hashPairs`. Pure.
 *
 * `state` is constant on a candidate (only `working`/`stopped` survive the
 * filter), so the tuple carries `pane_id`, `title`, `created_at`, and `job_id`
 * — enough that any change to which window gets which name re-fires the gate.
 */
 ...
 * winner's title verbatim (a spawn-name title like `work::fn-1019.2` tabs as
 * `work::fn-1019.2`); the comparison uses that same target, so a window already
