 ...
 * resume token — the session UUID for Claude/Pi or the back-filled native ID for
 * Hermes (EMPTY when keeper never resolved one ⇒ not-resumable). A v4 reader
...
export interface RestoreAgent {
  ...
  window_index: number | null;
  created_at: number;
  /** Launching harness (`"claude"`/`"pi"`/`"hermes"`). ABSENT ⇒ claude
...
export function buildRestoreTier(
  ...
  for (const job of jobs) {
    ...
    const agent: RestoreAgent = {
      ...
      // back-filled native ID for Hermes, "" when not-resumable.
