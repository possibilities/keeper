...
function migrate(db: Database): void {
  ...
  try {
    db.transaction(() => {
      ...
      // v113→v114 (fn-1172 task .3): retire the epic-level close-time
      ...
      // projection set so the post-migrate boot drain re-folds them under the v114
      ...
      // surface) — this bump MUST add 114 to `SUPPORTED_SCHEMA_VERSIONS` in
      // `keeper/api.py` in the SAME commit; test/schema-version.test.ts enforces it.
      if (preMigrateStoredVersion < 114) {
