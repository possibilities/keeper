...
| `observation-missing`, `observation-stale`, `pool-unavailable`, `pressure-contended`, `routing-error` | Capacity or the transient launch-pressure ledger cannot supply a healthy initial route for the requested quota scope. `pool-unavailable` includes no authorized alias, missing scope usage evidence such as absent Spark windows, scope quota cooldown/exhaustion, or account-wide auth/rate/transport cooldown; `pressure-contended` is only the owner-lock/ledger contention path; `routing-error` is a private routing-state fault. | Restore the observer/aliases/policy or let bounded contention clear, then inspect fresh `accounts check --json`; native Codex remains authoritative meanwhile. | yes after fresh evidence |
| `proof-missing`, `proof-invalid`, `proof-incomplete`, `proof-failed` | The attested report is absent, malformed/unknown/unsanitized, mixed-scope, incomplete/stale/interrupted, or fails because its declared verdict cannot be re-derived from the recorded transcript, exact top-level scope, route scopes, and bindings. No activation write occurs. | Preserve the refusal, restore healthy native state, rerun the managed proof probe for one scope, and stage its new allowlisted report. | yes with new evidence |
