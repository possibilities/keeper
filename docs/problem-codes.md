...
| code                   | meaning                                                                           | recovery                                                                                                                                                                                  | retry-safe        |
| ---------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `poison`               | A dead letter failed classification and blocks the commit rail globally or within the trusted producer-evidence scope shown by Needs-human. | After repairing the producer, run `keeper dead-letter reclassify <dl-id>`. If irrecoverable, run `keeper dead-letter resolve <dl-id> --force --reason <reason>`. | reclassify: yes; resolve: deliberate |
...
`--json`. Every non-active or uncertain state makes native `openai-codex` authoritative. Routing codes are
reported for the requested quota scope; scope authorization and usage evidence are separate eligibility
inputs from account-wide pressure/auth/rate/transport cooldowns.

| code | meaning | recovery | retry-safe |
| --- | --- | --- | --- |
| `activation-pending` | No passing live proof has activated the current revision/configuration/opaque alias set for the requested scope. | Complete the separate live proof, stage it with `proof capture`, inspect `proof verdict`, then run `activate`. | yes after new evidence |
...
| `observation-missing`, `observation-stale`, `pool-unavailable`, `pressure-contended`, `routing-error` | Capacity or the transient launch-pressure ledger cannot supply a healthy initial route for the requested quota scope. `pool-unavailable` includes no authorized alias, no capability-supported alias (Spark requires its exact observed quota window), scope quota cooldown/exhaustion, or account-wide auth/rate/transport cooldown; `pressure-contended` is only the owner-lock/ledger contention path; `routing-error` is a private routing-state fault. | Restore the observer/aliases/policy or let bounded contention clear, then inspect fresh `accounts check --json`. Generic traffic may use visible native fallback; managed Spark traffic fails closed rather than invoking an account outside its proven capability subset. | yes after fresh evidence |
| `proof-missing`, `proof-invalid`, `proof-incomplete`, `proof-failed` | The attested report is absent, malformed/unknown/unsanitized, mixed-scope, routes through an alias without attested scope capability, is incomplete/stale/interrupted, or declares a verdict that cannot be re-derived from the transcript, capability subset, exact scope, and bindings. No activation write occurs. | Preserve the refusal, restore fresh observations, rerun the managed proof probe for one scope, and stage its new allowlisted report. | yes with new evidence |
...
| `off-cube-triple`    | `claude_default`, `dispatch.work`, `dispatch.handoff`, `panel 'x' member 1` | A well-formed `<harness>::<model>::<effort>` triple names a combination the roster does not enumerate. | Correct the triple in `presets.yaml`/`panel.yaml`, or add the model/effort to `matrix.yaml`. | yes (read-only) |
...
| `stale_attempts` / `stale-pending`                                                     | An unbound Dispatch attempt exceeded its launch-window ceiling, either as a surviving pending launch or as an orphaned claim whose durable Dispatch claim outlived the ephemeral pending row across a crash. A delayed start gains no authority unless its exact attempt identity still owns the current claim. | Inspect the target's `dispatch_claims`, any surviving `pending_dispatches` row, and its dispatch-failure row, then `keeper autopilot retry <verb::id>`. The clear self-refuses (`refused_live`) while the claim is bound to a live or uncertain worker, so TERM-confirming the claimant dead stops being a manual precondition; `--force` overrides that liveness fence ONLY, never the exact-attempt identity CAS (a re-minted attempt returns `refused_identity`, which force cannot clear). |
...
### Trunk-lease integration (`close-finalize`)

`close-finalize` merges each live epic base into its repo's local default branch
under an independently fenced per-repo trunk lease. The merge itself runs in a
**private scratch worktree** cut from the leased default tip, never in the
shared checkout (ADR 0102, `docs/adr/0102-private-worktree-trunk-integration.md`):
...
fast-forward — the merge still lands locally via a compare-and-swap ref
advance, leaving the shared checkout trailing for the existing
shared-checkout-desync producer — rather than refusing the whole integration.
The scratch worktree and its temp branch are removed on every exit path below.

| code                            | meaning                                                                                                                                                                                                                       | recovery                                                                                                        |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `TRUNK_LEASE_REQUEST_FAILED`     | The daemon-published lease request could not be published for this repo.                                                                                                                                                    | Confirm the daemon is reachable and re-run `/plan:close`.                                                       |
| `TRUNK_LEASE_PENDING`            | The lease request was not published before the bounded wait expired — a live holder is likely blocking it.                                                                                                                  | Release any live holder, then re-run `/plan:close`.                                                             |
| `TRUNK_LEASE_RELEASE_FAILED`     | The daemon did not acknowledge release of a held lease; no successor may integrate until claimant death is observed.                                                                                                        | Wait for daemon-side claimant-death reclaim; do not hand-edit the lease leaf.                                   |
| `TRUNK_LEASE_IDENTITY_MISSING`   | A worktree-mode close has no tracked session identity, so ownerless trunk integration is refused.                                                                                                                           | Re-run from a session that carries a tracked identity (an ordinary `/plan:close` dispatch).                     |
| `TRUNK_INTEGRATION_LOCK_TIMEOUT` | The shared commit-work lock stayed occupied through the bounded deadline; no merge was attempted.                                                                                                                            | Wait for the concurrent commit / base-merge to finish, then re-run `/plan:close`.                               |
| `TRUNK_INTEGRATION_DEFERRED`     | A precondition for this fenced attempt could not be established — an unresolved source/default tip, an inconclusive ancestry re-probe, a scratch worktree that could not be provisioned, an unreachable/misidentified repo checkout, or a merge-suite gate that could not run. No merge was attempted or landed. | Re-run `/plan:close`; each cause is retried fresh from a new fence and freshly observed git state.              |
| `TRUNK_INTEGRATION_CONFLICT`     | The merge conflicted inside the private scratch worktree. The closer retains its fenced lease (no release) for the downstream resolver; no `MERGE_HEAD` ever reaches the shared checkout.                                  | Route through the close skill's Phase 1b (`plan:deconflicter`); never resolve by hand outside the claimed incident. |
| `TRUNK_INTEGRATION_FAILED`       | `git merge` failed in the scratch worktree without leaving merge residue (no `MERGE_HEAD`).                                                                                                                                  | Inspect the reported git output; the scratch worktree was already reaped and the lease released.                |
| `TRUNK_INTEGRATION_UNVERIFIED`   | `git merge` reported success but the source branch is not provably contained in the merged tree.                                                                                                                            | Investigate the scratch-worktree merge result before re-running; this indicates an unexpected git state.        |
| `TRUNK_INTEGRATION_SUITE_RED`    | The merge-suite gate failed against the merged tree in the scratch worktree; the land was aborted before touching the shared checkout.                                                                                      | Fix the semantic-merge breakage the gate reported, then re-run `/plan:close`.                                   |
| `TRUNK_TIP_DRIFT`                | The local default branch advanced during all three independently fenced lease attempts; no stale merge was attempted.                                                                                                       | Re-run `/plan:close` once the concurrent default-branch activity settles.                                       |
...
| `repeated-native-crash`       | Two or more boots in the restart-ledger window carry platform-confirmed native-crash attribution; unlike `daemon-crash-loop`, this also catches spaced crashes.                    | Inspect the named DiagnosticReports evidence and daemon stderr, then repair or replace the crashing runtime. The row clears when the attributed ledger window drains below two boots.                           | automatic after the evidence window drains; `retry_dispatch` does not apply |
...
A free-form operator lands Task work by claiming the exact Task in the same
tracked operator session, previewing with
`keeper commit-work --task-id <claimed-task> --preview-files`, then invoking
`keeper commit-work --task-id <claimed-task> <message>` directly. The literal
`--task-id` is first in both calls. Keeper validates the live exact-task session
marker and appends the mechanical `Task:` trailer consumed by close-preflight.
A caller-written trailer, a generic subagent handoff, raw Git, and
`KEEPER_PLAN_GUARD_BYPASS` are not
...
| `identity_untrusted`, `task_unbound`                                                                                                    | The UUID is not a working tracked Claude/Pi job on this invocation's exact `(pid, start_time)` ancestor chain, or `--task-id` is neither that work job's bound Task nor the free-form operator session's live exact-task marker.                                                                                      | Invoke from the owning active tracked session. A free-form operator must claim the Task in that same session before commit-work; a copied UUID, marker, or environment carrier from a sibling is not authority.                                                       | yes (nothing committed)     |
...
| `message_required`, `forbidden_trailer`                                                                                                 | A commit message is absent or contains any caller-supplied authority trailer, including `Task:`.                                                                                                                                                                | Supply plain prose and use the authorized `--task-id` from the bound work job or live exact-task operator claim; Keeper appends identity/task authority mechanically.                                                                                                | yes                         |
