...
| `ownership_conflict`                                                                                                                    | A selected/adopted path has a genuinely live or otherwise unknown foreign exclusive claimant.                                                                                                                                                                   | Let that claimant land or become positively terminal, or read the envelope's `request_release` pointer — it names the claimant and contested paths and carries a `keeper session release` invocation. Advise it as one bounded, best-effort bus notice; never signal or terminate a live claimant. Wait the grace window, then retry; a still-live conflict on retry escalates through the existing block ladder. A `decline` on the pointer is durable and terminal — back off under the attempt budget instead of re-asking.                                                                                          | yes after ownership settles or release |
