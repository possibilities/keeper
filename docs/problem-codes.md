...
| `stale_attempts` / `stale-pending`                                                     | A Dispatch attempt exceeded the launch-window ceiling. After a crash, its durable Dispatch claim may survive even when the ephemeral pending row does not; a delayed start gains no authority unless its attempt identity still matches the current claim.          | Inspect the target's `dispatch_claims`, any surviving `pending_dispatches` row, and its dispatch-failure row. Let the expiry/reconciler path fence it; retry a sticky failure only after confirming no current exact Dispatch claim or live activity owns the target. |
