...
| `ownership_conflict` | A selected/adopted path has a genuinely live or otherwise unknown foreign exclusive claimant. | Let that claimant land or become positively terminal; do not force or broaden the path set. | yes after ownership settles |
...
| `commit_state_indeterminate` | The bounded ref-CAS process timed out or lost complete output, so Keeper cannot prove whether the exact commit became visible. | Inspect the named commit SHA and captured branch before any retry; never recreate or roll back the commit blindly. | conditional |
