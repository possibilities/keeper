...
1. **Hard single-instance gate.** Main acquires a kernel `flock(2)` (`LOCK_EX|LOCK_NB`, via the existing `usage-flock` FileLock primitive) on a dedicated `keeperd.lock` file at the top of `startDaemon()`, before `openDb()`, `migrate()`, or any worker spawn. The fd lives for the process lifetime, carries `FD_CLOEXEC`, and is owned by module scope on main — no worker thread may close it. A live incumbent fails the boot closed with exit 1; an inconclusive primitive (dlopen failure, exotic FS) also fails closed with a distinct non-zero exit code. Both diagnostics name the lock path and print the `launchctl kickstart` recovery line. Socket and lock unlinks are ownership-checked so a dying stray can never unlink a live daemon's socket.
...
- A second daemon dies in milliseconds, before touching the DB, and an inconclusive lock attempt also exits before DB open — the second-writer/second-migrator window is closed on both admission edges.
- A persistently broken lock primitive wedges every boot loudly on launchd-captured stderr. Because admission exits before boot-ledger identity and append, these failures are intentionally absent from restart-ledger crash-loop distress; richer board attribution remains outside this decision.
