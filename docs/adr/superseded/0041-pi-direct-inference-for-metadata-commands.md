...
- snapshot Pi's bounded, compaction-aware model context as soon as it contains a user or assistant message, without waiting for the active turn to settle; only an empty context remains pending for an `agent_settled` retry.
...
- Metadata commands remain pending only while the active context contains no usable user or assistant text; manual title changes and session replacement cancel that pending intent rather than being overwritten.
