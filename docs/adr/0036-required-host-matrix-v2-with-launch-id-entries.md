...
closure is unchanged. The template renderer reads the host matrix directly
and fans `subagent_templates` members out over `subagent_models × efforts`
to the fixed `workers/<model>-<effort>` convention, retiring the `render_to:`
frontmatter (its sole consumer duplicated the code-owned constant); path
traversal guards move to the inventory entries and the derived capability
token. CI drift gates re-scope to what the repo can self-verify (hash
