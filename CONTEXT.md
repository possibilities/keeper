...
- **Capability model**: The model-axis value a task carries, a bare capability token the host matrix config resolves at run time to a serving harness and native model id; native when the claude provider carries it, wrapped otherwise. Avoid: harness model, backend model, model alias.
...
- **Alias target**: The provider-native id a capability model resolves to in a Provider's host matrix entry, optionally provider-qualified (`provider/native-id`) for a harness that rejects a bare capability id; carried verbatim to the launch model flag as pass-through. Distinct from a Launch triple (the context-free `harness::model::effort` launch token) — an alias target lives inside one matrix Provider entry, never a standalone name. Avoid: model alias, native alias, preset.
