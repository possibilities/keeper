...
- **Needs-human**: The family of board signals requiring operator attention — dead letters, block escalations, parked questions, and stuck dispatches, with finalize-non-ff and the instant-death wall as subsets of stuck dispatches that never double-count into the total. Avoid: alert queue, error state, attention list.
