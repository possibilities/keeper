...
- **Claude session**: One agent conversation with its own transcript, identified by an immutable session id; jobs and forensics key on it, while its human-renamable display title is a convenience lookup, never an identity. Avoid: job, chat, conversation; session name (for the title).
