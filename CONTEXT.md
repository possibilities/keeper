...
- **Needs-human**: The family of board signals requiring operator attention — dead letters, block escalations, parked questions, and stuck dispatches, with finalize-non-ff, the instant-death wall, and homed blocked-work rows as subsets of stuck dispatches that never double-count into the total, plus display-only members like the selection review and the pinned epic that never count toward the jam total at all. Avoid: alert queue, error state, attention list.
