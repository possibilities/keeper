...
# Recent per-job Task-tool subagent timeline — one row per PreToolUse:Agent paired with its PostToolUse:Agent (and lifecycle Start/Stop), status running|ok|failed|unknown|superseded, duration_ms populated on SubagentStop (NULL while the turn is still OPEN — including an `ok` row whose PostToolUse:Agent landed before its SubagentStop, a backgrounded sub still in flight — plus superseded peers + lifecycle-swept unknown orphans; canonical in-flight test = NULL duration_ms AND status in running|ok). Every plan worker shares subagent_type='work:worker' (the launcher selects the per-cell `work` plugin via --plugin-dir, so the spawn name is uniform and this row carries no model/effort) — recover the cell by joining the owning job's plan_ref (`work::fn-N.M`) to that task's tier + model; a WORK_CELL_KEY env stamp is a cheap optional add if inline per-cell telemetry is later wanted:
sqlite3 ~/.local/state/keeper/keeper.db \
